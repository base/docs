...
```typescript
{
   ...
   * the purchasing userâ€™s country of residence, e.g., US.
   */
  country: string;
  /**
   * ISO 3166-2 two-letter country subdivision code representing
   * the purchasing userâ€™s subdivision of residence within their country,
   * e.g. NY. Required if the country=â€œUSâ€ because certain states (e.g., NY)
