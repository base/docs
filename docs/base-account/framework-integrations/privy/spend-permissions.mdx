            ...
            {permissions.map((permission, index) => (
              ...
              >
                <div className="text-sm">
                  <div><strong>Spender:</strong> {permission.spender?.slice(0, 10)}...</div>
                  <div><strong>Token:</strong> {permission.token?.slice(0, 10)}...</div>
                  <div><strong>Allowance:</strong> {permission.allowance?.toString()}</div>
                </div>
              </div>
            ))}
          </div>
...
- **ETH**: Native token (no address needed)
