...
[Reown AppKit](https://reown.com/appkit) (formerly WalletConnect Web3Modal) is a powerful library for adding wallet connections to your onchain applications. It provides a polished modal interface with support for multiple wallets, email authentication, and social logins.

By integrating Reown with Base Account, you can provide users with seamless onboarding through Base's native smart wallet while maintaining access to other connection methods.
...
Reown provides several options to control which wallets appear in the modal:

```typescript
createAppKit({
  // ...
  // Featured wallets appear at the top of the modal
  featuredWalletIds: [BASE_ACCOUNT_WALLET_ID],

  // Only these wallets appear in "All Wallets" and as recommended
  includeWalletIds: [BASE_ACCOUNT_WALLET_ID],

  // Control the "All Wallets" button: 'SHOW' | 'HIDE' | 'ONLY_MOBILE'
  allWallets: 'SHOW',

  // Enable/disable injected wallets and WalletConnect
  enableWallets: true,

  // Enable/disable WalletConnect QR code
  enableWalletConnect: true,
})
```

### Connection Methods Order

Control the order of connection methods in the modal:
...
createAppKit({
  ...
  themeVariables: {
    '--w3m-accent': '#0052FF',        // Primary accent color
    '--w3m-color-mix': '#0052FF',     // Color mix for gradients
    '--w3m-color-mix-strength': 20,   // Strength of color mixing
    '--w3m-border-radius-master': '2px', // Border radius
  },
})
```
...
<Card title="Use Environment Variables" icon="shield-check">
  Store your Reown project ID in environment variables:
...
<Card title="Enable SSR Support" icon="server">
  Always enable SSR in your Wagmi adapter for Next.js applications:
...
<Card title="Prioritize Base Chain" icon="link">
  Put Base as the first chain in your networks configuration:
...
<CardGroup cols={2}>
  <Card title="Explore Base Account Features" icon="code" href="/base-account/overview">
    Learn more about Base Account capabilities
  </Card>
  <Card title="Reown Documentation" icon="book" href="https://docs.reown.com/appkit/react/core/installation">
    Explore the full Reown AppKit documentation
  </Card>
  <Card title="Wagmi Hooks" icon="hook" href="https://wagmi.sh/react/hooks">
    Learn about wagmi hooks for wallet interactions
  </Card>
  <Card title="Join Base Community" icon="users" href="https://discord.com/invite/buildonbase">
