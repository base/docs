...
- `100` – Pending: the wallet accepted the batch and is still processing it.
- `200` – Success: the batch executed onchain without reverts.
- `400` – Offchain error: the wallet failed the batch and will not retry.
- `500` – Chain error: the entire batch reverted onchain.
- `600` – Partial error: some calls reverted; review `receipts` for details.

When you receive a response, check `status.atomic` to determine whether the receipts array represents a single atomic transaction or multiple sequential transactions.
