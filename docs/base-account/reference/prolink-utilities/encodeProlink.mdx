...
Create shareable payment links for your users:
...
async function createPaymentLink(
  ...
  return `https://pay.yourapp.com?prolink=${prolink}`;
}
```

### QR Code Generation

Encode transactions into QR codes:

```typescript
import QRCode from 'qrcode';

async function generatePaymentQR(transaction: ProlinkRequest): Promise<string> {
  const prolink = await encodeProlink(transaction);
  const qrCodeDataUrl = await QRCode.toDataURL(prolink, {
...
Launch your app with pre-filled transaction data:

```typescript
async function createDeepLink(transaction: ProlinkRequest): Promise<string> {
  const prolink = await encodeProlink(transaction);
  return `yourapp://transaction?prolink=${prolink}`;
}
```

## Error Handling

The function throws errors for invalid inputs. Always wrap calls in try-catch:
...
try {
  ...
  if (error instanceof Error) {
    ...
    if (error.message.includes('requires chainId')) {
      console.error('Chain ID is required for this method');
    } else if (error.message.includes('requires params array')) {
...
<ResponseField name="wallet_sendCalls requires params array with at least one element">
  The params array is empty or not provided.
</ResponseField>

<ResponseField name="wallet_sendCalls params[0] must be an object">
  The first element of params is not an object.
</ResponseField>

<ResponseField name="wallet_sendCalls requires chainId in params">
  The chainId field is missing from params[0].
</ResponseField>

<ResponseField name="wallet_sign requires chainId in params">
  The chainId field is missing from params[0].
</ResponseField>

<ResponseField name="Invalid capabilities JSON">
  The capabilities object contains invalid JSON when serialized.
</ResponseField>
