...
description: "Retrieve permissions for a specific spender and chain, optionally filtered by account"
---

Coinbase-specific RPC method

<Info>
Retrieves permissions for a specific spender and chain. If an account is provided, returns only permissions for that account. Otherwise, returns all permissions for the spender. This method excludes permissions that have expired or been revoked, returning only active spend permissions.
</Info>
...
An array of permission objects, ordered from oldest to newest (older permissions appear first).
</ResponseField>
