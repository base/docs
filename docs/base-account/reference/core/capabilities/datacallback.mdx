...
type DataCallbackEvent = {
  type: 'initiated' | 'preSign' | 'postSign';
  context?: Record<string, unknown>;
  requests?: DataCallbackRequestType[]; // Only for preSign events
}

type DataCallbackCapability = {
  callbackURL?: string;
  events: DataCallbackEvent[];
}
```
    ...
    capabilities: {
      dataCallback: {
        ...
        events: [
          {
            type: "initiated",
            context: { orderId: "123" } // Optional context data
          },
          {
            type: "preSign",
            requests: [
            ...
            context: { shippingTier: "express" } // Optional context data
          },
          {
            type: "postSign",
            context: { webhookTag: "after" } // Optional context data
          }
        ]
      },
    },
  ...
  capabilities: {
    dataCallback: {
      ...
      events: Array<{
        ...
        context?: Record<string, unknown>;
        requests?: Array<{
          type: 'email' | 'phoneNumber' | 'physicalAddress' | 'name';
          optional?: boolean;
        }>;
      }>;
      requestedInfo?: {
