...
```typescript Monitor Subscription Lifecycle
import { base } from '@base-org/account';

async function monitorSubscription(subscriptionId: string) {
  try {
    const status = await base.subscription.getStatus({
      id: subscriptionId,
      testnet: false
    ...
    const now = new Date();
    const nextPeriod = new Date(status.nextPeriodStart!);
    const daysUntilRenewal = Math.ceil((nextPeriod.getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
    
    console.log("✅ Subscription active");
    console.log(`📅 Period: ${status.periodInDays} days`);
    console.log(`💰 Charge: $${status.recurringCharge}`);
    console.log(`💳 Available to charge: $${status.remainingChargeInPeriod}`);
    console.log(`⏰ Days until renewal: ${daysUntilRenewal}`);
    
    return {
      active: true,
      canCharge: parseFloat(status.remainingChargeInPeriod || '0') > 0,
      daysUntilRenewal
    };
  } catch (error) {
