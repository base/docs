...
const expectedDomain = 'yourapp.com';
const nonces = new Set<string>(); // Populate this from your /auth/nonce route.

export async function verifySig(req, res) {
  const { address, message, signature } = req.body;
  const { nonce } = parseSiweMessage(message);

  // Check the nonce against your server-side store before accepting it.
...
app.post("/auth/verify", async (req, res) => {
  ...
  // 1. Parse the SIWE message and check that the nonce exists server-side
  ...
  // 2. Verify the signature and the expected SIWE fields
  const valid = await client.verifySiweMessage({
    address,
    message,
    signature,
