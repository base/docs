...
A malicious user could submit the same valid transaction ID multiple times to receive goods or services repeatedly. Always track processed transaction IDs in your database.
</Warning>
...
### Validate user info with dataCallback
...
When you provide a `callbackURL` in your `payerInfo` configuration, the wallet will POST the user's information to your endpoint for validation. This gives you the opportunity to:

- Verify email addresses are valid and not disposable
- Validate shipping addresses against your supported regions
- Check phone numbers are correctly formatted
- Block suspicious or fraudulent information

```ts Client-side setup
const payment = await pay({
  amount: '25.00',
  to: '0xRecipient',
  payerInfo: {
    ...
    callbackURL: 'https://your-api.com/validate-payment-info'
  }
});
```
