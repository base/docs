...
This callback will return a [LifecycleStatusData](/builderkits/onchainkit/checkout/checkout#advanced-usage) object including the [TransactionReceipt](https://github.com/wevm/viem/blob/main/src/types/transaction.ts#L38) and `chargeId`.
...
To customize the name and logo of your application rendered in the popup, set the `name` and `logo` values inside [OnchainKitProvider](/builderkits/onchainkit/config/onchainkit-provider#usage).
...
You can use our Checkout [`LifecycleStatus`](/builderkits/onchainkit/checkout/types#lifecyclestatus) and the `onStatus` prop to listen to transaction states.
...
- [`LifecycleStatus`](/builderkits/onchainkit/checkout/types#lifecyclestatus)
- [`CheckoutReact`](/builderkits/onchainkit/checkout/types#checkoutreact)
- [`CheckoutButtonReact`](/builderkits/onchainkit/checkout/types#checkoutbuttonreact)
- [`CheckoutStatusReact`](/builderkits/onchainkit/checkout/types#checkoutstatusreact)
