...
Spend Permissions enable third-party signers to spend assets (native and ERC-20 tokens) from a user's Base Account.
...
<Steps>
  ...
  Developer Platform API Key, you don't need one for this example. When prompted to use Coinbase Base Account select "yes".
  ...
  to connect their Base Account to the application.
  ...
  <Step title="Configure the Base Account URL">
  ...
  When you click the "Subscribe" button you should be prompted to create or connect your Base Account.

  You can create a new Base Account via the popup. Note that you'll need a little ETH in this wallet to fund the
  ...
  - prompts the user to connect their Coinbase Base Account to our app
