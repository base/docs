...
In this guide, we'll explore how to use the Profiles feature in Base Account to request and validate user information
...
- Set up a NextJS+Wagmi project to work with Base Account Profiles
...
<GithubRepoCard title="Base Account Profiles Demo" githubUrl="https://github.com/base/demos/tree/master/smart-wallet/profiles-demo" />

## Understanding Profiles

The Profiles feature allows developers to request personal information from Base Account users during a transaction. This is useful for applications that need:
...
Base Account handles the collection of this information and it is left to you to validate it and store it as you see fit
...
Let's create a simple app that demonstrates the Profiles feature. We'll be using NextJS with [Wagmi](https://wagmi.sh/) hooks to integrate with Base Account.
...
Base Account requires callback URLs to use HTTPS, even for local development.
...
Now, let's create an API endpoint to validate the data received from Base Account.
This endpoint will check for valid formats and return errors if needed.

<Info>
**Wallet Validation**

Base Account will make some basic validation of the data before sending it to your callback URL.
This includes checking that the email is a valid email address and the physical address is a valid address.

If the data is invalid, Base Account will return an error to the user and not make the callback.
...
When a user visits your application and connects their Base Account, the following process occurs:

1. **User connects their Base Account**: The user clicks "Sign in with Base Account" and authorizes the connection.
...
3. **Base Account prompts the user**: The wallet shows a UI asking the user to share the requested information.

4. **User provides data**: The user enters/confirms their information in the Base Account interface.

5. **Data is validated**: Base Account sends the data to your callback URL for validation.
...
When a user provides profile information, Base Account returns a response object with the following structure:
...
Base Account will show these error messages to the user and prompt them to correct the information.
...
5. Connect your Base Account and test requesting and validating user data.
...
Optional fields will be marked as such in the Base Account interface, allowing users to skip them.
...
The Profiles feature in Base Account provides a secure and user-friendly way to collect necessary user information for your onchain app. By following this guide, you've learned how to:

- Request profile data from Base Account users
