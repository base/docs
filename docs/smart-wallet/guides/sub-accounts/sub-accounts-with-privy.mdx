...
description: 'How to enable Base Account Sub Accounts in a Privy project'
---
...
In this guide, you'll learn how to enable [Base Account Sub Accounts](/smart-wallet/concepts/features/optional/sub-accounts) in a [Privy](https://www.privy.io/) project.
...
- Enable automatic Base Account Sub Account creation for users authenticating with Privy
...
- [Base Account Sub Accounts](/smart-wallet/concepts/features/optional/sub-accounts) are **only available on the canary version** of the Coinbase Wallet SDK and on **Base Sepolia**
    ...
    externalWallets: {
      coinbaseWallet: {
        config: {
          ...
          preference: {
            keysUrl: "https://keys-dev.coinbase.com/connect", // Use the Base Account dev environment
            options: "smartWalletOnly", // Only allow Base Account
          },
...
- `keysUrl`: Points to the Base Account development environment. **Sub Accounts only work in this environment.**
...
- `defaultSpendLimits`: Sets Spend Permissions from the global Base Account to the Sub Account on Base Sepolia (`84532`).
...
This guide only covers the _automatic_ Sub Account creation flow, where a Sub Account is created and Spend Permissions are set as soon as the user connects their wallet. We are working with Privy to support flows where the user can first authenticate with their global Base Account account and then create a Sub Account after. For now, only the automatic Sub Accounts flow is supported.
</Note>
...
- [Base Account Sub Accounts Feature](/smart-wallet/concepts/features/optional/sub-accounts)
