...
Once you've [set up your Wagmi config and created your Base Account connection](/smart-wallet/guides/sub-accounts/setup), you can use it to perform various operations like signing messages and sending transactions.
...
To send transactions with a Sub Account, we'll use Wagmi's `useSendTransaction` hook. Remember that transactions will be limited by the Spend Permissions we configured in the [setup](/smart-wallet/guides/sub-accounts/setup):
...
1. Connect their Base Account (Sign in with Base Account button)
...
All transactions will be automatically limited by the Spend Permissions configured in your [Wagmi setup](/smart-wallet/guides/sub-accounts/setup).
