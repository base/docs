...
Users will see Base Account and mobile app connection options.
...
With this option, users will only see an option to create a Base Account or sign into their Base Account.
...
This option only applies to Coinbase Base Account. When a valid data suffix is supplied, it is appended to the initCode and executeBatch calldata.
...
If auto is true, the Base Account will generate a 16 byte hex string from the apps origin.

##### `dataSuffix` (optional)

- Type: `0x${string}`

Base Account expects a 16 byte hex string. If the data suffix is not a 16 byte hex string, the Base Account will ignore the property.
