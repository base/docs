...
description: Learn step-by-step on how to turn a regular template app into an onchain app with a wallet connection.
...
As discussed above, add `'use client';` to the top of the `providers.tsx` file.
