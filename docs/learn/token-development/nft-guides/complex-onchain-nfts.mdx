    ...
    bytes(
      string(
        abi.encodePacked(
          ...
          "Build the SVG with the token ID as the seed",
          '"}'
        )
      )
    )
...
```text
{"name": "Land, Sea, and Sky #: 1", "description": "Land, Sea, and Sky is a collection of generative art pieces stored entirely onchain.", "image": "data:image/SVG+xml;base64,Build the SVG with the token ID as the seed"}
```
