...
You also have to be careful that you write code with predictable execution paths. Each transaction is sent with a gas limit which various frameworks, such as _ethers.js_, do their best to estimate. If this estimate is wrong, the transaction will fail, but **it will still cost the gas used up until the point it failed!**
