...
export const TradeExecutionDemo = () => {
  ...
  const examples = [
    {
      ...
      events: [
        ...
        { delay: 250, type: "approval", url: "keys.coinbase.com/approve/req_abc123" },
    ...
    },
    {
      ...
      events: [
        ...
        { delay: 250, type: "approval", url: "keys.coinbase.com/approve/req_degen01" },
    ...
    },
    {
      ...
      events: [
        ...
        { delay: 250, type: "approval", url: "keys.coinbase.com/approve/req_eth01" },
  ...
  const UserBubble = ({ children }) => (
    <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 20 }}>
      <div className="ted-bubble" style={{ background: c.bubble, color: c.bubbleText, padding: "12px 16px", borderRadius: 14, fontFamily: sans, lineHeight: 1.45, border: `1px solid ${c.toolBorder}` }}>{children}</div>
    </div>
  );

  const ToolCall = ({ tool, completed }) => (
    <div style={{ marginBottom: 10 }}>
      <div className="ted-tool-chip" style={{ display: "inline-flex", alignItems: "flex-start", gap: 8, background: c.toolBg, border: `1px solid ${c.toolBorder}`, borderRadius: 8, padding: "6px 11px", opacity: completed ? 0.85 : 1 }}>
        ...
        <span className="ted-tool-text" style={{ fontFamily: mono, color: c.muted }}>
  ...
  const Thinking = () => (
    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14, fontFamily: sans, fontSize: 13, color: c.muted }}>
      <span style={{ display: "inline-flex", gap: 3 }}>
        {[0, 1, 2].map(i => <span key={i} style={{ width: 5, height: 5, borderRadius: "50%", background: c.muted, opacity: 0.4, animation: `ted-pulse 1.2s infinite ${i * 0.18}s` }} />)}
      </span>
  ...
  const ApprovalLink = ({ url }) => (
    <div style={{ marginBottom: 10, marginTop: 4 }}>
      <span className="ted-approval" style={{ display: "inline-flex", alignItems: "flex-start", gap: 8, background: c.toolBg, border: `1px solid ${c.accent}`, borderRadius: 8, padding: "9px 13px", fontFamily: mono, color: c.accent, boxShadow: `0 0 0 3px rgba(217,119,87,0.08)`, maxWidth: "100%" }}>
  ...
  const ChipBtn = ({ onClick, children }) => {
    ...
    return (
      <button onClick={onClick} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)} className="ted-chip"
  ...
  return (
    <div style={{ margin: "28px 0", borderRadius: 14, overflow: "hidden", border: `1px solid ${c.border}`, background: c.bg, boxShadow: "0 8px 24px rgba(0,0,0,0.25)" }}>
      <style>{`
        @keyframes ted-pulse{0%,100%{opacity:0.3;transform:scale(1)}50%{opacity:1;transform:scale(1.3)}}
        .ted-chat{height:380px;padding:24px 28px 16px}
        .ted-input-row{padding:10px 16px 14px}
        .ted-tool-text{white-space:nowrap;font-size:12px;line-height:1.4}
        .ted-tool-chip{max-width:100%}
        .ted-bubble{max-width:78%;font-size:14px}
        .ted-approval{font-size:12.5px}
        .ted-chip{padding:16px 18px;font-size:15px}
        .ted-empty-text{font-size:16px}
        .ted-footnote{font-size:11px}
        @media(max-width:640px){
          .ted-chat{height:440px;padding:16px 14px 12px}
          .ted-input-row{padding:8px 10px 10px}
          .ted-tool-chip{display:block}
          .ted-tool-text{white-space:normal;word-break:break-word;font-size:11px}
          .ted-bubble{max-width:88%;font-size:13.5px}
          .ted-approval{font-size:11.5px;word-break:break-all}
          .ted-chip{padding:14px 14px;font-size:14px}
          .ted-empty-text{font-size:14.5px}
          .ted-footnote{font-size:10.5px}
        }
      `}</style>
      ...
      <div ref={scrollRef} className="ted-chat" style={{ overflowY: "auto" }}>
        {!ex && (
          <div>
            <div className="ted-empty-text" style={{ fontFamily: serif, color: c.muted, marginBottom: 20, lineHeight: 1.5 }}>
              Send tokens to any address or name once <span style={{ fontFamily: mono, fontSize: "0.85em", color: c.code, background: c.codeBg, padding: "1px 6px", borderRadius: 4 }}>mcp.base.org</span> is connected:
            </div>
