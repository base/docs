...
export const AgentRegistrationDemo = () => {
  ...
  const examples = [
    {
      ...
      events: [
        ...
        { delay: 250, type: "rows", rows: [
          ...
          { token: "Swapped",        amount: "−$180.41", value: "100 USDC → 0.035 ETH · 1 hr ago" },
          { token: "Received USDC",  amount: "+$50.00",  value: "from coinbase.com · 3 hr ago" },
          { token: "Sent 5 USDC",    amount: "−$5.00",   value: "bob.eth · 1 day ago" },
          { token: "Received USDC",  amount: "+$100.00", value: "from 0x9f3a…2e01 · 2 days ago" },
        ]},
      ],
    },
    {
      ...
      events: [
        ...
        { delay: 250, type: "rows", rows: [
          ...
          { token: "marketplace.base",  amount: "−25 USDC",  value: "3 days ago" },
        ]},
        { delay: 400, type: "confirm", text: "Total sent: 40 USDC over 3 transactions" },
      ],
    },
    {
  ...
  const UserBubble = ({ children }) => (
    <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 20 }}>
      <div className="ard-bubble" style={{ background: c.bubble, color: c.bubbleText, padding: "12px 16px", borderRadius: 14, fontFamily: sans, lineHeight: 1.45, border: `1px solid ${c.toolBorder}` }}>{children}</div>
    </div>
  );

  const ToolCall = ({ tool, completed }) => (
    <div style={{ marginBottom: 10 }}>
      <div className="ard-tool-chip" style={{ display: "inline-flex", alignItems: "flex-start", gap: 8, background: c.toolBg, border: `1px solid ${c.toolBorder}`, borderRadius: 8, padding: "6px 11px", opacity: completed ? 0.85 : 1 }}>
        ...
        <span className="ard-tool-text" style={{ fontFamily: mono, color: c.muted }}>
  ...
  const Thinking = () => (
    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14, fontFamily: sans, fontSize: 13, color: c.muted }}>
      <span style={{ display: "inline-flex", gap: 3 }}>
        {[0, 1, 2].map(i => <span key={i} style={{ width: 5, height: 5, borderRadius: "50%", background: c.muted, opacity: 0.4, animation: `ard-pulse 1.2s infinite ${i * 0.18}s` }} />)}
      </span>
  ...
  const ResponseRows = ({ rows }) => (
    <div style={{ marginBottom: 14 }}>
      {rows.map((r, i) => (
        <div key={i} className="ard-row" style={{ display: "flex", alignItems: "baseline", padding: "5px 0", fontFamily: serif, fontSize: 14, color: c.body }}>
          <span style={{ minWidth: 12, color: c.dim, flexShrink: 0 }}>•</span>
          <span className="ard-row-token" style={{ fontWeight: 500 }}>{r.token}</span>
  ...
  const ChipBtn = ({ onClick, children }) => {
    ...
    return (
      <button onClick={onClick} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)} className="ard-chip"
  ...
  return (
    <div style={{ margin: "28px 0", borderRadius: 14, overflow: "hidden", border: `1px solid ${c.border}`, background: c.bg, boxShadow: "0 8px 24px rgba(0,0,0,0.25)" }}>
      <style>{`
        @keyframes ard-pulse{0%,100%{opacity:0.3;transform:scale(1)}50%{opacity:1;transform:scale(1.3)}}
        .ard-chat{height:400px;padding:24px 28px 16px}
        .ard-input-row{padding:10px 16px 14px}
        .ard-tool-text{white-space:nowrap;font-size:12px;line-height:1.4}
        .ard-tool-chip{max-width:100%}
        .ard-row{gap:12px;flex-wrap:nowrap}
        .ard-row-token{min-width:160px}
        .ard-bubble{max-width:78%;font-size:14px}
        .ard-chip{padding:16px 18px;font-size:15px}
        .ard-empty-text{font-size:16px}
        .ard-footnote{font-size:11px}
        @media(max-width:640px){
          .ard-chat{height:460px;padding:16px 14px 12px}
          .ard-input-row{padding:8px 10px 10px}
          .ard-tool-chip{display:block}
          .ard-tool-text{white-space:normal;word-break:break-word;font-size:11px}
          .ard-row{flex-wrap:wrap;gap:4px 10px}
          .ard-row-token{min-width:100%;flex:1 1 100%}
          .ard-bubble{max-width:88%;font-size:13.5px}
          .ard-chip{padding:14px 14px;font-size:14px}
          .ard-empty-text{font-size:14.5px}
          .ard-footnote{font-size:10.5px}
        }
      `}</style>
      ...
      <div ref={scrollRef} className="ard-chat" style={{ overflowY: "auto" }}>
        {!ex && (
          <div>
            <div className="ard-empty-text" style={{ fontFamily: serif, color: c.muted, marginBottom: 20, lineHeight: 1.5 }}>
              Ask your assistant about your transaction history once <span style={{ fontFamily: mono, fontSize: "0.85em", color: c.code, background: c.codeBg, padding: "1px 6px", borderRadius: 4 }}>mcp.base.org</span> is connected:
            </div>
      ...
      <div className="ard-input-row">
        ...
        <div className="ard-footnote" style={{ textAlign: "center", marginTop: 8, fontFamily: sans, color: c.dim }}>
