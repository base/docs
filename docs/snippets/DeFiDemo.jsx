  ...
  return (
    <div className="wf" style={{ margin: "22px 0", maxWidth: 760, borderRadius: 8, border: `1px solid ${C.border}`, background: C.white, overflow: "hidden", boxShadow: "0 1px 2px rgba(10,11,13,.04)" }}>
        ...
        <div className="wf-rail" style={{ padding: "16px 16px 14px", background: C.white }}>
          ...
          {f.readout && sim.metrics.length > 0 && (
            <div style={{ marginTop: 14, paddingTop: 12, borderTop: `1px solid ${C.border}` }}>
              <div style={{ fontFamily: sans, fontSize: 10, fontWeight: 600, letterSpacing: ".6px", textTransform: "uppercase", color: C.sub, marginBottom: 8 }}>Illustrative position</div>
              <div style={{ display: "grid", gap: 6 }}>
                {sim.metrics.map((m) => (
                  <div key={m.label} style={{ display: "flex", alignItems: "center", gap: 8, fontFamily: sans, fontSize: 12.5, color: C.body }}>
                    <span style={{ flex: 1 }}>{m.label}</span>
                    <span style={{ fontFamily: mono, fontSize: 12.5, fontWeight: 600, color: m.tone === "warn" ? C.error : m.tone === "ok" ? C.success : C.ink }}>{m.value}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      <div style={{ padding: "10px 16px", background: C.panel, borderTop: `1px solid ${C.border}`, display: "flex", alignItems: "center", gap: 10 }}>
        <span style={{ fontFamily: sans, fontSize: 11, color: C.sub, lineHeight: 1.4 }}>{f.footer}</span>
      </div>
    </div>
  );
};
