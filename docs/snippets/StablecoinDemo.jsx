export const StablecoinDemo = ({ flow }) => {
  ...
  const dot = { Issuer: "var(--sd-accent)", Alice: "#66c800", Bob: "#ffd12f", Merchant: "#3c8aff" };
  ...
  const FLOWS = {
    issue: {
      ...
      steps: [
        { text: "Create a fiat-backed token. Name, currency, and admin are set at creation.", action: "Create token",
        ...
        { text: "It's live and fully ERC-20 compatible.", action: "Confirm",
    ...
    mint: {
      ...
      steps: [
        { text: "1,000 in fiat lands in reserves. Mint matching supply.", action: "Mint 1,000",
          run: (s) => { s.balances.Alice = (s.balances.Alice || 0) + 1000; return { entries: [ok("Transfer", "0x0 → Alice · 1,000")] }; } },
        { text: "Cap supply so circulation can't exceed reserves.", action: "Cap at 1,200",
          run: () => ({ entries: [ok("SupplyCapUpdated", "cap 1,200")] }) },
        { text: "A mint past the cap is rejected by the protocol.", action: "Try minting 500",
    ...
    burn: {
      ...
      steps: [
        { text: "Alice holds 1,000 aUSD.", action: "Fund Alice",
          run: (s) => { s.balances.Alice = 1000; return { entries: [ok("Transfer", "0x0 → Alice · 1,000")] }; } },
        { text: "Alice redeems 400 for fiat and returns the tokens.", action: "Return 400",
          run: (s) => { s.balances.Alice -= 400; s.balances.Issuer = (s.balances.Issuer || 0) + 400; return { entries: [ok("Transfer", "Alice → Issuer · 400")] }; } },
        { text: "Retire the returned tokens so supply matches reserves.", action: "Burn 400",
    ...
    restrict: {
      ...
      steps: [
        { text: "Turn on your KYC allowlist. Approve Alice and your merchant.", action: "Enable allowlist",
        ...
        { text: "Approved accounts transact normally.", action: "Alice pays merchant",
        ...
        { text: "An account you haven't approved is turned away.", action: "Try paying Bob",
    ...
    block: {
      ...
      steps: [
        { text: "Bob holds 50 aUSD.", action: "Mint to Bob",
          run: (s) => { s.balances.Bob = 50; return { entries: [ok("Transfer", "0x0 → Bob · 50")] }; } },
        { text: "A compliance hold comes in for Bob's address.", action: "Block address",
        ...
        { text: "Bob can no longer move funds.", action: "Bob tries to pay",
          run: () => ({ entries: [err("PolicyForbids", "TRANSFER_SENDER · Bob")], caption: "Blocked by the protocol, not by custom contract code." }) },
        { text: "When the hold clears, unblock the address.", action: "Unblock",
    ...
    recover: {
      ...
      steps: [
        { text: "Bob's address is blocked and holds 50 aUSD.", action: "Set up",
          run: (s) => { s.balances.Bob = 50; s.blocked = "Bob"; return { entries: [ok("Transfer", "0x0 → Bob · 50"), ok("updateBlocklist", "add Bob")] }; } },
        { text: "A holder lost their keys. Reclaim the balance.", action: "Reclaim funds",
        ...
        { text: "Reissue to the holder's new address.", action: "Reissue",
    ...
    pause: {
      ...
      steps: [
        { text: "Alice holds 100 aUSD. Everything is running normally.", action: "Fund Alice",
          run: (s) => { s.balances.Alice = 100; return { entries: [ok("Transfer", "0x0 → Alice · 100")] }; } },
        { text: "An incident hits. Halt transfers instantly.", action: "Pause transfers",
          run: () => ({ entries: [ok("Paused", "TRANSFER")], caption: "Only transfers stop. Pausing is granular." }) },
        { text: "No one can move funds while transfers are paused.", action: "Try a transfer",
          run: () => ({ entries: [err("EnforcedPause", "TRANSFER is paused")] }) },
        { text: "Resume once the incident is resolved.", action: "Resume",
    ...
    reconcile: {
      ...
      steps: [
        { text: "Alice pays your processor 25 aUSD, tagged with the invoice.", action: "Pay with memo",
        ...
        { text: "The back office matches the payment to the order.", action: "Match payment",
      ...
      <div style={{ padding: "14px 16px 6px" }}>
        ...
        {f.readout && holders.length > 0 && (
          <div style={{ marginTop: 4, marginBottom: 10, padding: "9px 12px", background: c.panel, border: `1px solid ${c.border}`, borderRadius: 10 }}>
            <div style={{ fontFamily: sans, fontSize: 9.5, fontWeight: 600, letterSpacing: "0.9px", color: c.dim, textTransform: "uppercase", marginBottom: 6 }}>Balances</div>
