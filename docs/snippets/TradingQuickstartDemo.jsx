export const TradingQuickstartDemo = () => {
  ...
  // confirms the swap-review modal, then resumes with the execution + result.
  const examples = [
    {
      prompt: "Swap 100 USDC for ETH on Base",
      events: [
        { delay: 380, type: "thinking" },
        { delay: 460, type: "text", text: "I'll pull a quote and route for the pair, then pause for your approval before swapping." },
        { delay: 650, type: "tool", tool: { server: "base-mcp", name: "get_swap_quote", desc: "Fetches a swap quote and best route for the token pair.", args: { sellToken: "USDC", buyToken: "ETH", sellAmount: "100", chain: "base" } } },
        { delay: 520, type: "text", text: "Quote ready: 100 USDC → 0.03512 ETH · price impact 0.09%. Approve the swap to continue:" },
        ...
        { delay: 520, type: "tool", tool: { server: "base-mcp", name: "swap", desc: "Executes the swap through the router with the approved route.", args: { sellToken: "USDC", buyToken: "ETH", sellAmount: "100", chain: "base" } } },
        { delay: 460, type: "text", text: "Swap confirmed on Base. Summary:" },
        { delay: 220, type: "rows", cols: ["field", "value", "detail"], rows: [
          ["Sold", "100 USDC", "from 0x71Dc…7244"],
          ["Received", "0.03512 ETH", "price impact 0.09%"],
          ["Tx", "0x9f2a…4c1e", "confirmed on Base"],
        ] },
        { delay: 380, type: "confirm", text: "Swapped 100 USDC → 0.03512 ETH." },
      ],
    },
    {
      ...
      events: [
        { delay: 380, type: "thinking" },
        { delay: 460, type: "text", text: "I'll quote the reverse pair and settle it only after you approve." },
        { delay: 650, type: "tool", tool: { server: "base-mcp", name: "get_swap_quote", desc: "Fetches a swap quote and best route for the token pair.", args: { sellToken: "ETH", buyToken: "USDC", sellAmount: "0.05", chain: "base" } } },
        { delay: 520, type: "text", text: "Swap request ready: 0.05 ETH → an estimated 179.41 USDC. Approve the swap to continue:" },
        { delay: 220, type: "approval", preview: {
          title: "Approve swap", subtitle: "You are authorizing a token swap on Base.",
        ...
        { delay: 520, type: "tool", tool: { server: "base-mcp", name: "swap", desc: "Executes the swap through the router with the approved route.", args: { sellToken: "ETH", buyToken: "USDC", sellAmount: "0.05", chain: "base" } } },
        { delay: 460, type: "text", text: "Swap confirmed on Base. Summary:" },
        { delay: 220, type: "rows", cols: ["field", "value", "detail"], rows: [
          ["Sold", "0.05 ETH", "from 0x71Dc…7244"],
          ["Received", "179.41 USDC", "estimated at quote"],
          ["Tx", "0x3b8e…a027", "confirmed on Base"],
        ] },
        { delay: 380, type: "confirm", text: "Swapped 0.05 ETH → 179.41 USDC." },
      ],
    },
    {
      ...
      events: [
        { delay: 380, type: "thinking" },
        { delay: 460, type: "text", text: "I'll resolve the cbBTC token first, then quote the swap and pause for approval." },
        { delay: 560, type: "tool", tool: { server: "base-mcp", name: "search_tokens", desc: "Resolves a token symbol to its canonical address on Base.", args: { query: "cbBTC", chain: "base" } } },
        { delay: 620, type: "tool", tool: { server: "base-mcp", name: "get_swap_quote", desc: "Fetches a swap quote and best route for the token pair.", args: { sellToken: "USDC", buyToken: "cbBTC", sellAmount: "200", chain: "base" } } },
        { delay: 520, type: "text", text: "Quote ready: 200 USDC → 0.00210 cbBTC at current rates. Approve the swap to continue:" },
        { delay: 220, type: "approval", preview: {
          title: "Approve swap", subtitle: "You are authorizing a token swap on Base.",
        ...
        { delay: 520, type: "tool", tool: { server: "base-mcp", name: "swap", desc: "Executes the swap through the router with the approved route.", args: { sellToken: "USDC", buyToken: "cbBTC", sellAmount: "200", chain: "base" } } },
        { delay: 460, type: "text", text: "Swap confirmed on Base. Summary:" },
        { delay: 220, type: "rows", cols: ["field", "value", "detail"], rows: [
          ["Sold", "200 USDC", "from 0x71Dc…7244"],
          ["Received", "0.00210 cbBTC", "~$199.50 at rate"],
          ["Tx", "0x5c14…9df3", "confirmed on Base"],
        ] },
        { delay: 380, type: "confirm", text: "Swapped 200 USDC → 0.00210 cbBTC." },
      ],
    },
  ];
...
{"{\n"}{Object.entries(tool.args).map(([k, v], i, a) => (
  ...
  // ---- generated swap-review modal (light) ----
      ...
      <div onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true" style={{ background: C.white, borderRadius: 8, border: `1px solid ${C.border}`, width: 360, maxWidth: "100%", maxHeight: "calc(100% - 16px)", overflowY: "auto", boxShadow: "0 24px 64px rgba(10,11,13,.24)" }}>
        ...
        {preview.amount && (
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, padding: "12px 18px", borderTop: `1px solid ${C.border}` }}>
            <span style={{ fontFamily: sans, fontSize: 12.5, color: C.sec }}>Sell</span>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 18px", borderTop: `1px solid ${C.border}` }}>
          ...
          <span style={{ fontFamily: sans, fontSize: 10.5, fontWeight: 600, letterSpacing: ".3px", color: C.blue }}>DEMO · not a real transaction</span>
        </div>
  ...
  return (
    <div className="as" style={{ position: "relative", margin: "22px 0", maxWidth: 760, borderRadius: 8, border: `1px solid ${C.border}`, background: C.white, overflow: "hidden", boxShadow: "0 1px 2px rgba(10,11,13,.04)" }}>
      ...
      <div ref={scrollRef} className="as-body">
        {!ex ? (
          <div className="as-land">
            <div>
              <div style={{ fontFamily: sans, fontSize: 20, fontWeight: 600, letterSpacing: "-0.02em", color: C.ink, lineHeight: 1.25 }}>Trade any token on Base</div>
              <div style={{ fontFamily: sans, fontSize: 13.5, color: C.sec, lineHeight: 1.5, marginTop: 8 }}>
                Ask the assistant to quote and swap tokens through <span style={{ fontFamily: mono, fontSize: "0.92em", color: C.blue, background: C.blueSoft, padding: "1px 5px", borderRadius: 4 }}>mcp.base.org</span>. It pauses for your approval before any swap is submitted.
              </div>
            </div>
            <div>
      ...
      <div style={{ padding: "12px 16px 14px", borderTop: `1px solid ${C.border}`, background: C.white }}>
        ...
        <div style={{ textAlign: "center", marginTop: 8, fontFamily: sans, fontSize: 11, color: C.sub }}>Demo · every swap requires your approval in Base Account — AI responses can make mistakes.</div>
      </div>
    </div>
  );
};
