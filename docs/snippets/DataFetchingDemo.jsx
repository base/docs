...
export const DataFetchingDemo = () => {
  ...
  const steps = [
    { delay: 350, left: [{ t: "> Discover price feed service...", c: "active" }], right: [
      { t: "── Service Discovery ───────────────────", c: "dim" },
      { t: "GET /api/discover?query=price+feed", c: "code" },
      { t: "Host: api.wallet.paysponge.com", c: "muted" },
    ]},
    { delay: 700, left: [{ t: "  ← found: api.coingecko.com · 0.001 USDC", c: "muted" }], right: [
      { t: "", c: "dim" },
      { t: '{"id":"coingecko-price",', c: "code" },
      { t: '  "baseUrl":"api.coingecko.com",', c: "code" },
      { t: '  "price":"0.001 USDC/req"}', c: "code" },
    ]},
    { delay: 500, left: [{ t: "> Fetch ETH price...", c: "active" }], right: [
      { t: "", c: "dim" },
      { t: "── API Request ─────────────────────────", c: "dim" },
      { t: "GET /api/v3/simple/price?ids=ethereum", c: "code" },
    ]},
    { delay: 600, left: [{ t: "  ← 402 · paying 0.001 USDC...", c: "muted" }], right: [
      { t: "", c: "dim" },
      { t: "HTTP/1.1 402 Payment Required", c: "muted" },
      { t: "X-Payment-Sig: pending...", c: "muted" },
    ]},
    { delay: 700, left: [{ t: "  ✓ payment sent · retrying...", c: "success" }], right: [
      { t: "", c: "dim" },
      { t: "── API Response ────────────────────────", c: "dim" },
      { t: "HTTP/1.1 200 OK", c: "success" },
    ]},
    { delay: 500, left: [{ t: "  ← 200 OK · data returned", c: "success" }], right: [
      { t: '{"ethereum":{', c: "code" },
      { t: '  "usd": 2847.32,', c: "code" },
      { t: '  "usd_24h_change": 2.31', c: "code" },
      { t: "} }", c: "code" },
    ]},
    { delay: 350, left: [{ t: "  ETH  $2,847.32  ↑ 2.3%", c: "code", bold: true }], right: [] },
  ];
  ...
  return (
    <div style={{ margin: "28px 0", borderRadius: 12, overflow: "hidden", border: "1px solid #27272a", background: "#09090b" }}>
      <div style={{ display: "flex", alignItems: "center", padding: "8px 12px", background: "#111113", borderBottom: "1px solid #27272a" }}>
        ...
        <span style={{ fontFamily: mono, fontSize: 12, color: "#52525b" }}>Data Fetching</span>
        <div style={{ flex: 1 }} />
