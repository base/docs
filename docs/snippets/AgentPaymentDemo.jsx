export const AgentPaymentDemo = () => {
  ...
  // Conversation script. An agent inspects its wallets, checks balances, and
  // sends USDC — pausing on an `approval` event until the user confirms the
  // transaction-review modal, then resumes.
  const examples = [
    {
      prompt: "Show me my agent's wallets",
      events: [
        { delay: 380, type: "thinking" },
        { delay: 460, type: "text", text: "I'll list the wallets connected to your agent through the Base MCP." },
        { delay: 650, type: "tool", tool: { server: "base-mcp", name: "get_wallets", desc: "Lists the wallets your agent can operate.", args: {} } },
        { delay: 460, type: "text", text: "Your agent has 2 wallets connected:" },
        { delay: 220, type: "rows", cols: ["wallet", "address", "status"], rows: [
          ["Agent wallet", "0x9c2d…e4f8", "spend policy on"],
          ["Base Account", "0x4a3f…b7c1", "owner · approves"],
        ] },
        { delay: 380, type: "confirm", text: "Connected — ready to send, swap, and sign." },
      ],
    },
    {
      prompt: "What's my agent's USDC balance on Base?",
      events: [
        { delay: 380, type: "thinking" },
        { delay: 460, type: "text", text: "Let me pull the agent wallet's balances on Base." },
        { delay: 650, type: "tool", tool: { server: "base-mcp", name: "get_portfolio", desc: "Returns token balances and USD values for the agent wallet.", args: { address: "0x9c2d…e4f8", chain: "base" } } },
        { delay: 460, type: "text", text: "Current balances on Base:" },
        { delay: 220, type: "rows", cols: ["token", "amount", "value"], rows: [
          ["USDC", "245.80", "$245.80"],
          ["ETH", "0.0412", "$148.33"],
          ["WETH", "0.0100", "$36.02"],
        ] },
        { delay: 380, type: "confirm", text: "Total: $430.15 on Base." },
      ],
    },
    {
      ...
      events: [
        { delay: 380, type: "thinking" },
        { delay: 460, type: "text", text: "I'll prepare a USDC transfer from the agent wallet. It stays paused until you approve." },
        { delay: 650, type: "tool", tool: { server: "base-mcp", name: "send", desc: "Prepares a USDC transfer for your approval.", args: { recipient: "alice.base.eth", asset: "USDC", amount: "5", chain: "base" } } },
        { delay: 520, type: "text", text: "Resolved alice.base.eth → 0x71C7…976F. This is within your spend policy — approve to send:" },
        ...
        { delay: 520, type: "tool", tool: { server: "base-mcp", name: "send", desc: "Submits the approved transfer to Base.", args: { recipient: "0x71C7…976F", asset: "USDC", amount: "5", chain: "base" } } },
        { delay: 460, type: "text", text: "Transfer submitted. Receipt:" },
        { delay: 220, type: "rows", cols: ["field", "value", "detail"], rows: [
          ["Status", "Success", "confirmed on Base"],
          ["Amount", "5.00 USDC", "to alice.base.eth"],
          ["Tx hash", "0x8f2a…c19d", "1 block confirmation"],
        ] },
        { delay: 380, type: "confirm", text: "Sent 5 USDC to alice.base.eth." },
      ],
    },
  ];
...
{"{"}{Object.keys(tool.args).length === 0 ? "}" : "\n"}{Object.entries(tool.args).map(([k, v], i, a) => (
  ...
  // ---- generated transaction-review modal (light) — "Approve transaction" (send) ----
    ...
    <div className="as" style={{ position: "relative", margin: "22px 0", maxWidth: 760, borderRadius: 8, border: `1px solid ${C.border}`, background: C.white, overflow: "hidden", boxShadow: "0 1px 2px rgba(10,11,13,.04)" }}>
      ...
      <div ref={scrollRef} className="as-body">
        {!ex ? (
          <div className="as-land">
            <div>
              <div style={{ fontFamily: sans, fontSize: 20, fontWeight: 600, letterSpacing: "-0.02em", color: C.ink, lineHeight: 1.25 }}>Let your agent pay on Base</div>
              <div style={{ fontFamily: sans, fontSize: 13.5, color: C.sec, lineHeight: 1.5, marginTop: 8 }}>
                Ask your agent to check balances and send USDC through <span style={{ fontFamily: mono, fontSize: "0.92em", color: C.blue, background: C.blueSoft, padding: "1px 5px", borderRadius: 4 }}>mcp.base.org</span>. Every payment pauses for your approval before any USDC leaves the wallet.
              </div>
            </div>
            <div>
      ...
      <div style={{ padding: "12px 16px 14px", borderTop: `1px solid ${C.border}`, background: C.white }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, background: C.panel, border: `1px solid ${C.border}`, borderRadius: 6, padding: "8px 10px 8px 12px" }}>
          ...
          <span style={{ flex: 1, fontFamily: sans, fontSize: 13.5, color: C.sub, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>Ask your agent to check a balance or send a payment…</span>
        ...
        <div style={{ textAlign: "center", marginTop: 8, fontFamily: sans, fontSize: 11, color: C.sub }}>Demo · every payment requires your approval in Base Account — AI responses can make mistakes.</div>
      </div>
    </div>
  );
};
