export const AcceptingPaymentsDemo = () => {
  ...
  // Conversation script for a merchant accepting USDC on Base. Read-only flows
  // (creating a charge, verifying a payment) run straight through. The settle
  // flow pauses on an `approval` event until the user confirms the
  // transaction-review modal, then resumes.
  const examples = [
    {
      prompt: "Create a USDC payment request for 50 USDC",
      events: [
        { delay: 380, type: "thinking" },
        { delay: 460, type: "text", text: "I'll create a Base payment request you can share with your customer." },
        { delay: 650, type: "tool", tool: { server: "base-mcp", name: "create_charge", desc: "Creates a USDC payment request and returns a shareable link.", args: { amount: "50.00", asset: "USDC", chain: "base" } } },
        { delay: 520, type: "text", text: "Your charge is live. Share this link to collect payment:" },
        { delay: 220, type: "rows", cols: ["field", "value", "detail"], rows: [
          ["Charge", "chg_8f2a91", "created"],
          ["Amount", "50.00 USDC", "on Base"],
          ["Link", "base.pay/chg_8f2a91", "share to collect"],
          ["Status", "awaiting payment", "expires in 24h"],
        ] },
        { delay: 380, type: "confirm", text: "Payment request created — 50.00 USDC on Base." },
      ],
    },
    {
      prompt: "Has charge chg_8f2a91 been paid yet?",
      events: [
        { delay: 380, type: "thinking" },
        { delay: 460, type: "text", text: "Let me check for an incoming USDC transfer matching that charge." },
        { delay: 650, type: "tool", tool: { server: "base-mcp", name: "get_transaction_history", desc: "Reads recent transfers to your receiving address.", args: { address: "0x71Dc…7244", asset: "USDC", direction: "incoming" } } },
        { delay: 520, type: "text", text: "Payment received and confirmed onchain:" },
        { delay: 220, type: "rows", cols: ["field", "value", "detail"], rows: [
          ["Charge", "chg_8f2a91", "matched"],
          ["Received", "50.00 USDC", "from 0x9aE2…1b30"],
          ["Confirmations", "12", "final on Base"],
          ["Tx", "0x4c7f…a2e1", "view onchain"],
        ] },
        { delay: 380, type: "confirm", text: "Charge chg_8f2a91 is paid — 50.00 USDC received." },
      ],
    },
    {
      prompt: "Sweep my received USDC to the treasury wallet",
      events: [
        { delay: 380, type: "thinking" },
        { delay: 460, type: "text", text: "I'll move your settled USDC balance to your treasury address." },
        { delay: 650, type: "tool", tool: { server: "base-mcp", name: "get_portfolio", desc: "Reads the settled USDC balance available to sweep.", args: { address: "0x71Dc…7244", asset: "USDC" } } },
        { delay: 520, type: "text", text: "You have 312.50 USDC settled. Approve the transfer to your treasury to continue:" },
        ...
        { delay: 520, type: "tool", tool: { server: "base-mcp", name: "send", desc: "Signs and submits the USDC transfer to your treasury.", args: { to: "0x4F2b…9C10", amount: "312.50", asset: "USDC" } } },
        { delay: 460, type: "text", text: "Transfer confirmed. Summary:" },
        { delay: 220, type: "rows", cols: ["field", "value", "detail"], rows: [
          ["Sent", "312.50 USDC", "to treasury"],
          ["Tx", "0x8b3d…77af", "confirmed on Base"],
          ["Fee", "$0.004", "paid in ETH"],
        ] },
        { delay: 380, type: "confirm", text: "Swept 312.50 USDC to treasury." },
      ],
    },
  ];
    ...
    <div onClick={onCancel} className="as-anim" style={{ position: "absolute", inset: 0, zIndex: 50, background: "rgba(10,11,13,.55)", display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      <div onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true" style={{ background: C.white, borderRadius: 8, border: `1px solid ${C.border}`, width: 360, maxWidth: "100%", maxHeight: "calc(100% - 16px)", overflowY: "auto", boxShadow: "0 24px 64px rgba(10,11,13,.24)" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 18px", borderTop: `1px solid ${C.border}` }}>
          ...
          <span className="wf-t-caption" style={{ color: C.blue }}>DEMO · not a real signature</span>
        </div>
  ...
  return (
    <div className="as" style={{ position: "relative", margin: "22px 0", maxWidth: 760, borderRadius: 8, border: `1px solid ${C.border}`, background: C.white, overflow: "hidden", boxShadow: "var(--wf-shadow)" }}>
      ...
      <div ref={scrollRef} className="as-body">
        {!ex ? (
          <div className="as-land">
            <div>
              <div className="wf-t-title2" style={{ fontWeight: 600, color: C.ink }}>Accept USDC payments on Base</div>
              <div className="wf-t-body" style={{ color: C.sec, marginTop: 8 }}>
                Ask the assistant to create charges, verify incoming payments, and settle funds through <span style={{ fontFamily: mono, fontSize: "0.92em", color: C.blue, background: C.blueSoft, padding: "1px 5px", borderRadius: 4 }}>mcp.base.org</span>. It pauses for your signature before any USDC moves.
              </div>
            </div>
            <div>
      ...
      <div style={{ padding: "12px 16px 14px", borderTop: `1px solid ${C.border}`, background: C.white }}>
        ...
        <div className="wf-t-footnote" style={{ textAlign: "center", marginTop: 8, color: C.sub }}>Demo · transfers require a wallet signature — AI responses can make mistakes.</div>
      </div>
    </div>
  );
};
