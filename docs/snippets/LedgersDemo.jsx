  ...
  return (
    <div className="wf" style={{ margin: "22px 0", maxWidth: 760, borderRadius: 8, border: `1px solid ${C.border}`, background: C.white, overflow: "hidden", boxShadow: "var(--wf-shadow)" }}>
        ...
        <div className="wf-rail" style={{ padding: "16px 16px 14px", background: C.white }}>
          ...
          {f.readout && f.privacy && (
            <div style={{ marginTop: 14, paddingTop: 12, borderTop: `1px solid ${C.border}` }}>
              <div className="wf-t-caption" style={{ color: C.sub, marginBottom: 8 }}>What's exposed onchain</div>
              <div style={{ display: "grid", gap: 6 }}>
                {f.privacy.map((p) => {
                  const hidden = p.state === "hidden";
                  return (
                    <div key={p.label} className="wf-t-body" style={{ display: "flex", alignItems: "center", gap: 8, color: C.body }}>
                      ...
                      <span className="wf-t-caption" style={{ color: hidden ? C.blue : C.sub, border: `1px solid ${hidden ? C.blue : C.border}`, borderRadius: 4, padding: "0 4px" }}>{hidden ? "Hidden" : "Public"}</span>
                    </div>
                  );
                })}
              </div>
      ...
      <div style={{ padding: "10px 16px", background: C.panel, borderTop: `1px solid ${C.border}`, display: "flex", alignItems: "center", gap: 10 }}>
        <span className="wf-t-footnote" style={{ color: C.sub }}>{f.contrast}</span>
      </div>
    </div>
  );
};
