...
âŒ The data is processed by the function
 âœ… The function processes data
Conversational brevity - Use "you" but cut excess words.
âŒ You should configure the environment variables
 âœ… Configure your environment variables
Data over description - Show metrics, not marketing.
âŒ incredibly fast â†’ âœ… completes in < 100ms
...
**Direct constructions** - Start with the subject. Eliminate "There is/There are" and filler phrases (~~at this point in time~~ â†’ now, ~~is able to~~ â†’ can).
...
**Precise** - Use explicit names: âŒ "the auth file" â†’ âœ… "`authentication.js`"

**Clear** - Define abbreviations on first use: "Application Programming Interface (API)" â†’ then use "API"
