...
> - Implement shareable routes and embed entry points throughout your Mini App
...
Think of sharing as your app's built-in referral system. The better the embed looks and the more value it communicates, the more it spreads. 
...
<meta name="fc:frame" content='{
  "version":"next",
  "imageUrl":"https://your-app.com/embed-image",
  "button":{"title":"Launch App","action":{...}}
}' />
```
...
### Best Practices for Metadata-Driven Embeds
...
<Note>
If you're using MiniKit, this metadata setup can be automated  see [Using MiniKit for embeds](#using-minikit-for-embeds).
</Note>
...
Don't wait for users to decide to share  integrate sharing into key moments in your app's flow where users feel accomplished or want to showcase their achievements.
...
export default function ShareResultButton({ message, embedUrl }) {
...
<Tip>
Treat sharing as a celebration  users are most likely to share when they've accomplished something meaningful or want to show off their results.
</Tip>
...
```ts page.tsx
// Dynamic metadata for user-specific results
...
<Accordion title="Embeds not updating after changes">
  **Possible causes:**
  ...
  - Platform caching previous metadata
  - CDN or proxy caching issues
...
<Step title="Implement metadata generation">
  Add `fc:frame` metadata to your shareable pages using MiniKit's `generateMetadata` function or custom HTML meta tags.
</Step>
