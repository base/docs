---
title: 'Mini App Manifest (`farcaster.json`)'
...
- **Search indexing**: Your app appears in search after it has been shared at least once. Indexing typically completes in ~10 minutes. Set `"noindex": true` to exclude from search.
...
<Warning>
If your Mini App does not show in search, follow the debugging guide [here](/base-app/build-with-minikit/debugging#1-app-discovery--indexing-issues).
</Warning>
...
<Tip>
Use your app’s canonical domain in the payload. Keep this in sync with `frame.canonicalDomain` if you set it.
</Tip>
...
| `canonicalDomain` | string | No     | Canonical domain for the app. | Domain only (no protocol/port/path), max 1024 chars. |

#### Launch Experience

| Property                | Type   | Required | Description                | Constraints |
|-------------------------|--------|----------|----------------------------|-------------|
| `splashImageUrl`        | string | No       | Loading image.             | HTTPS URL, recommended 200×200px. |
| `splashBackgroundColor` | string | No       | Loading background color.  | Hex code (e.g., `#000000`). |

#### Discovery & Search

| Property           | Type     | Required | Description                                        | Constraints |
|--------------------|----------|----------|----------------------------------------------------|-------------|
| `primaryCategory`  | string   | No       | Controls where your app appears in category browsing. | One of: `games`, `social`, `finance`, `utility`, `productivity`, `health-fitness`, `news-media`, `music`, `shopping`, `education`, `developer-tools`, `entertainment`, `art-creativity`. |
| `tags`             | string[] | No       | Search/filter tags.                                | Up to 5; ≤ 20 chars each; lowercase; no spaces/emojis/special chars. |
...
| `requiredCapabilities` | string[] | No       | Required SDK capabilities.  | Must match Base capability keys. |
...
- All required fields are present (`frame.version`, `frame.name`, `frame.homeUrl`, `frame.iconUrl`)  
...
- If using `requiredChains`, values are valid CAIP‑2 IDs for Base networks  
...
- Review discovery mechanics and ranking in [Mini App Search and Discovery](/base-app/guides/search-and-discovery)  
- Learn how embeds are generated and tested in [Sharing Your Mini App](/base-app/guides/sharing-your-miniapp)  
- Explore the broader Mini App specification reference for visual mapping and examples
