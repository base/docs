...
<Steps>
  ...
  <Step title="Deploy a contract with Foundry">
    ...
    <Note>
    If `https://sepolia.base.org` is unreachable, use an alternative public endpoint such as `https://base-sepolia-rpc.publicnode.com`. This endpoint is also rate-limited and not recommended for production apps. For production apps, use a dedicated RPC provider.
    </Note>
