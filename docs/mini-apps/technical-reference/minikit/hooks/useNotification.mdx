...
<RequestExample>
```typescript Backend Proxy Route (Node.js/Express)
...
```python Backend Proxy Route (Python/FastAPI)
...
<RequestExample>
```tsx Game Achievement Notification
...
```tsx Tournament Reminder System
import { useNotification } from '@coinbase/onchainkit/minikit';
...
```tsx Social Update Notifications
