...
When your app is opened it can access information about the session from `sdk.context`. 

`sdk.context` provides 4 data objects:

- `user`: User's profile data
...
<Tip>Context is only rendered in Farcaster clients like the Base app and Farcaster. We highly encourage you to use `isInMiniApp()` hooks to handle different states.</Tip>
