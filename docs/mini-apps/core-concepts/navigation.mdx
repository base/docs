...
<Tab title="Farcaster SDK">
  ...
  Use [`useOpenUrl()`](/mini-apps/core-concepts/navigation) to safely open external websites in the client's in-app browser:
...
<Tabs>
  ...
  <Tab title="MiniKit">
  Use [`useComposeCast()`](/mini-apps/core-concepts/navigation) to open the native composer with prefilled content:
