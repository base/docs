...
//
//
[Display: "Cache: full"={status: Ready}, get if {"auth"=1} then {window: dialog="Press Button":"Yes"=1 or "No"="False"} ] 
["check Status auth account" if "cache":true then continue: "file start"]
//
