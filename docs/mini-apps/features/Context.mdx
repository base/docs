...
Context is the environmental data that describes how, where, and by whom your mini app was launched. This guide shows you how to leverage context to create personalized experiences and optimize viral growth.
...
Import and use the MiniKit hook to access context:
...
## Using context to determine experience

```typescript User Flow Logic
function UserExperience() {
  const { context } = useMiniKit();
  const isReturning = context?.client?.added || 
                     context?.location?.type === 'launcher';
  
  return isReturning ? <ReturningFlow /> : <OnboardingFlow />;
}
```

### Viral Loop Implementation

```typescript Viral Features
function ViralFeatures() {
  const { context } = useMiniKit();
  
  // Show share options for all entry points
  const showThanks = context?.location?.type === 'cast_embed';
  const showSavePrompt = !context?.client?.added;
  
  return (
    <div>
      {showThanks && <ThankSharer />}
      {showSavePrompt && <SaveAppPrompt />}
      <ShareButton />
    </div>
  );
}
