...
If you need to know whether a cast was successfully created or want to handle the response data, use the async version of the hook:

```ts AsyncComposeCastButton.tsx
import { useComposeCast } from '@coinbase/onchainkit/minikit';

export default function AsyncComposeCastButton() {
 const { composeCastAsync } = useComposeCast();

 const handleAsyncCompose = () => {
    composeCast({ text: 'Just completed a challenge!' });
  };


 const handleAsyncComposeWithEmbed = async () => {
   try {
     const castData = await composeCastAsync({
       text: 'Just completed a challenge!',
       embeds: [window.location.href],
     });
     
     // Handle successful cast creation
     console.log('Cast created successfully:', castData);
     // You can now update UI, track analytics, etc.
 ...
 return (
   <button onClick={handleAsyncCompose}>
     Share with Response Handling
   </button>
 );
}
```
