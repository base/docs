...
|----------|---------------------|---------------|
| **Apps needing instant UX** | Flashblocks-aware RPC with `pending` tag | [API Reference](/base-chain/flashblocks/api-reference) |
...
| **Standard apps** | Continue using regular RPCs | [JSON-RPC API Reference](/base-chain/reference/json-rpc-api) |
...
For HTTP and WebSocket endpoint URLs, see the [Flashblocks API Reference](/base-chain/flashblocks/api-reference#endpoints). Public endpoints are rate-limited — for production use, connect through a Flashblocks-enabled node provider such as Alchemy, QuickNode, or dRPC.
