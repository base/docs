...
<AccordionGroup>
  ...
  <Accordion title="What RPC methods support Flashblocks?">
    ...
    | `eth_call` | Use `pending` tag — the call result is correct, but `block.number` in context may appear a few blocks behind tip. See [FAQ](/base-chain/flashblocks/faq#why-does-eth_call-pending-report-a-block-number-several-blocks-behind-tip) for details. |
