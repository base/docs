...
<Info>
Most [standard JSON-RPC methods](/base-chain/reference/json-rpc-api) work as-is against a Flashblocks endpoint — use the `"pending"` block tag to read preconfirmed state. **Exception:** `eth_call "pending"` executes against stale state (3–5 blocks behind tip), not the live Flashblock. Use [`eth_simulateV1`](#eth_simulatev1) to simulate against preconfirmed state.
</Info>
