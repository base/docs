...
<Info>
This subscription requires a WebSocket-enabled RPC endpoint. The public Base endpoints (`mainnet.base.org`, `sepolia.base.org`) are HTTP only. Choose a Flashblocks-capable WebSocket provider from the [Base Services Hub](/get-started/base-services-hub).
</Info>
...
// Use a Flashblocks-enabled provider WSS endpoint (see the Base Services Hub)
