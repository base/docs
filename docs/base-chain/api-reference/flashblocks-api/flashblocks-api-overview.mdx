...
As of v0.9.0 (Azul), `new_account_balances` and `receipts` are no longer present in the `metadata` object. Only `block_number` remains.
...
<Note>
`metadata.receipts` was removed in v0.9.0 (Azul). This schema is preserved for reference if you are running an older node version. On v0.9.0+, use [`eth_getTransactionReceipt`](/base-chain/api-reference/ethereum-json-rpc-api/eth_getTransactionReceipt) for receipt data.
</Note>
