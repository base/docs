...
<Info>
This subscription requires a WebSocket-enabled RPC endpoint. The public Base endpoints (`mainnet.base.org`, `sepolia.base.org`) are HTTP only. Choose a WebSocket-capable provider from the [Base Services Hub](/get-started/base-services-hub).
</Info>
...
```json Notification (full: false) lines wrap expandable
{
  "jsonrpc": "2.0",
...
```json Notification (full: true) lines wrap expandable
{
  "jsonrpc": "2.0",
  "method": "eth_subscription",
  "params": {
