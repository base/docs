...
<Warning>
Debug methods replay transactions and are computationally expensive. Availability and rate limits vary among providers in the [Base Services Hub](/get-started/base-services-hub). Avoid calling these in hot paths.
</Warning>
...
<CodeGroup>
```json Request (default struct log) lines wrap expandable
{
  "jsonrpc": "2.0",
...
```json Request (callTracer) lines wrap expandable
{
  "jsonrpc": "2.0",
  "method": "debug_traceTransaction",
  "params": [
...
```json Response (default) lines wrap expandable
{
  "jsonrpc": "2.0",
  "id": 1,
  "result": {
...
```json Response (callTracer) lines wrap expandable
{
  "jsonrpc": "2.0",
  "id": 1,
  "result": {
