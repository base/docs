...
<Note>
**`eth_call "pending"` block context on Flashblocks nodes:** The call result is correct and reflects all pre-confirmed state. However, block-context properties (`block.number`, `block.timestamp`, `block.basefee`) may reflect a block several behind tip due to how nodes cache historical Flashblocks. Contracts that branch on these values should account for this. See the [FAQ](/base-chain/flashblocks/faq#why-does-eth_call-pending-report-a-block-context-several-blocks-behind-tip) for details.
</Note>
