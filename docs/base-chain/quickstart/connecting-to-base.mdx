...
<Info>
The public endpoints above are rate-limited and not suitable for production traffic. For production use, choose an RPC provider from the [Builder Stack](/get-started/builder-stack).
</Info>

<Note>
The public Base endpoints are **HTTP only**. WebSocket RPC connections (`eth_subscribe`, `newHeads`, `logs`) are not available on public Base endpoints — choose a WebSocket-capable provider from the [Builder Stack](/get-started/builder-stack).
</Note>
