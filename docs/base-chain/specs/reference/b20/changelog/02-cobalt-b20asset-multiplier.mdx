...
**Q: Can I send tokens to the token's own address?**
No. At Cobalt, `transfer`, `transferFrom`, `mint`, `mintWithMemo`, `batchMint`, and
`seizeWithMemo` all reject `to == address(this)` with `InvalidReceiver(to)`. A B20 token is a
precompile with no holder key, so a credit there is unrecoverable by the sender. `seizeWithMemo`
from the token address is still allowed, so a balance already stuck there can be recovered to a
treasury.
