...
- **Addresses**: `to != address(0)`, `to != address(this)`, and `from != to`, or the call reverts
  ...
  _may_ equal `address(this)`: seizing from the token's own address is permitted so that any balance
...
with no holder key, so no one could move the funds out. Seizing _from_ `address(this)` is permitted
