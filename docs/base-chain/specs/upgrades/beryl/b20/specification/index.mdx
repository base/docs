...
B20 uses an OpenZeppelin-style access-control model with fixed built-in roles.
...
Stablecoin tokens add `currency()`, set once at creation. The value must contain uppercase `A`-`Z` characters only and is self-declared, not externally verified.
