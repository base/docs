...
To deploy admin-less from block one, set `initialAdmin` to `address(0)` and put all required role grants, policy bindings, and cap configuration into initCalls. Create any policy that you will bind before calling `createB20`; initCalls execute on the new token and cannot create a policy in the singleton registry.
...
<Warning>
There is no later admin recovery path for an admin-less token. Missing token roles, policy bindings, or other token-admin configuration cannot be added after deployment. A bound policy remains independently manageable by its PolicyRegistry admin.
</Warning>
