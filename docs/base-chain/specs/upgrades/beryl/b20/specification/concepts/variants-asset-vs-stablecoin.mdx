...
<Warning>
Do not assume `balanceOf(account)` and `scaledBalanceOf(account)` are the same display unit. Integrations that show asset balances must decide whether they use raw ERC-20 balances or the scaled UI view.
</Warning>
...
<Warning>
The currency code is self-declared and not verified against any external registry.
</Warning>
