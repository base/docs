...
- Seizing with `SEIZE_ROLE` through `seizeWithMemo` when the holder is denied by `SEIZE_HOLDER_POLICY` and the destination is authorized by `SEIZE_RECEIVER_POLICY`.
...
`seizeWithMemo` transfers a holder's balance to a destination in one admin operation. It skips allowance and transfer policies, but requires the holder to be denied by `SEIZE_HOLDER_POLICY` and the destination to be authorized by `SEIZE_RECEIVER_POLICY`.

```solidity
// Configure seizure eligibility for holders and an authorized destination set.
...
// Later, an authorized operator seizes from a denied holder to an authorized destination.
