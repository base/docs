...
### Read a token's policy configuration
...
| `updatePolicy(scope, policyId)` | Sets the policy for `scope`. Admin-gated. Reverts if the scope is not recognized. |

## Check if a transfer will succeed

To confirm an account can move tokens, read each scope's policy ID from the token and check it against the [Policy Registry](#policy-registry), then check pause state:
