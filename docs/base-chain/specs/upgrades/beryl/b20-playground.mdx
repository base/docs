...
An underlying real-world asset may undergo events that change its redemption ratio - a corporate action, a distribution, or accrued yield. Updating the multiplier reflects these events onchain instantly, so holder balances track the underlying without a rebase, airdrop, or migration.
