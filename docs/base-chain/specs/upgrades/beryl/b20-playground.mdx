...
Your customer pays to one address. The transfer carries the invoice reference, so payment and order ID land in the same transaction. Reconciliation becomes a log query instead of a deposit address for every customer. To build this, see [Accept a B20 payment](/build-on-base/accept-payments/from-humans#accept-b20-with-a-memo).
...
<CardGroup cols={2}>
  <Card title="Launch a B20 token" href="/base-chain/network-information/b20-token-standard">
  ...
  <Card title="B20 specification" href="/base-chain/specs/reference/b20">
