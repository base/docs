...
description: "Execution engine changes in the Holocene upgrade, enabling dynamic EIP-1559 parameters encoded in block headers and configured via SystemConfig."
---
...
```rs Extended PayloadAttributesV3 lines wrap expandable
PayloadAttributesV3: {
    timestamp: QUANTITY
...
[prior EIP-1559 constants](/specifications/base-protocol/execution/l2-execution-engine#1559-parameters) must be used to populate `extraData` instead.
...
were [constants](/specifications/base-protocol/execution/l2-execution-engine#1559-parameters).
...
- if Holocene is not active in `parent_header.timestamp`, the [prior EIP-1559
  constants](/specifications/base-protocol/execution/l2-execution-engine#1559-parameters) are used. Note that `parent_header.extraData` is empty
