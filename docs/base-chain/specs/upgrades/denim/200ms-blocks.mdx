---
title: "200ms Blocks"
description: "Specification for Denim's canonical 200ms blocks, including BaseTime, derivation, validation, RPC behavior, and migration guidance."
tag: "Soon"
---

<Warning>
This specification is provided for informational and feedback purposes only. It does not constitute a commitment to ship any feature, on any timeline, or in this form. Integrators should not rely on this specification for production decisions until it is designated final.

Denim is not active on Base Sepolia or Base Mainnet. The upgrade is in planning for November 2026; exact activation timestamps and required client versions remain undecided.
</Warning>
...
| --- | --- | --- |
| `sepolia` | November 2026 (`TBD`) | `TBD` |
| `mainnet` | November 2026 (`TBD`) | `TBD` |

Exact activation timestamps have not been scheduled. A later node upgrade guide will provide release and rollout instructions.

## Motivation

Flashblocks expose sub-second updates while building within a single canonical block. Native 200ms blocks remove that indirection: every 200ms interval becomes a complete standalone block with its own number, hash, state root, receipts, and unsafe-to-finalized lifecycle. Applications retain a sub-second confirmation experience while consuming canonical block data.
...
Stop consuming Flashblocks-specific WebSocket streams and move to canonical block responses and subscriptions such as `eth_subscribe("newHeads")`. Existing contracts and applications remain compatible. Use the BaseTime `timestampMs()` getter only when a contract needs same-block millisecond precision; continue using `block.timestamp` when second-level precision is sufficient.

### RPC and Indexing Providers

Support the optional `timestampMs` field on blocks and headers and `blockTimestampMs` on mined transactions and logs. Preserve the fields when proxying or indexing responses, and do not infer millisecond timestamps from the whole-second header value.

### Node Operators

Upgrade to a Denim-compatible release before activation. Plan capacity for the higher native block rate across execution, storage, RPC events, and peer-to-peer propagation. Required versions and rollout instructions will be published in the Denim node upgrade guide.
