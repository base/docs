...
description: "Execution engine changes in the Isthmus upgrade, including L2ToL1MessagePasser storage root in block headers and operator fee collection."
---

[l2-to-l1-mp]: ../../protocol/execution/evm/predeploys.mdx#L2ToL1MessagePasser
[output-root]: ../../reference/glossary.mdx#l2-output-root
...
because it is a [proxied predeploy](../../protocol/execution/evm/predeploys.mdx) -- from genesis it
...
Similar to the `bn256Pairing` precompile in the [granite hardfork](../granite/exec-engine.mdx),
...
All of the BLS precompiles should be [accelerated](../../protocol/fault-proof/index.mdx#precompile-accelerators) in fault proof
...
`blobBaseFeeScalar` used in the [`L1Fee`](../../protocol/execution/index.mdx#ecotone-l1-cost-fee-changes-eip-4844-da).
...
These collected fees are sent to a new vault for the `operatorFee`: the [`OperatorFeeVault`](predeploys.mdx#operatorfeevault).
