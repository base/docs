...
description: "Specification of the Cannon Fault Proof Virtual Machine (FPVM), the multithreaded VM used for executing programs in dispute game resolution."
---
...
| hint response      | 3               | read-only. Used to read the status of [pre-image hinting](index.mdx#hinting). |
| hint request       | 4               | write-only. Used to provide [pre-image hints](index.mdx#hinting)              |
| pre-image response | 5               | read-only. Used to [read pre-images](index.mdx#pre-image-communication).      |
| pre-image request  | 6               | write-only. Used to [request pre-images](index.mdx#pre-image-communication).  |
