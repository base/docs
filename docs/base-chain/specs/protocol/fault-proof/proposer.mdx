---
title: "Proposer"
description: "Specification of the L2 output proposer, which publishes L2 output roots and withdrawal hashes to L1 for use in the fault proof system."
---

[g-rollup-node]: ../../reference/glossary.mdx#rollup-node
[g-mpt]: ../../reference/glossary.mdx#merkle-patricia-trie
[header-withdrawals-root]: ../../upgrades/isthmus/exec-engine.mdx#l2tol1messagepasser-storage-root-in-header
...
queries the [rollup node](../consensus/index.mdx) for the latest output root derived from the latest
[finalized](../consensus/index.mdx#finalization-guarantees) L1 block. It then takes the output root and
...
1. The `withdrawal_storage_root` (`bytes32`) elevates the Merkle-Patricia-Trie ([MPT][g-mpt]) root of the [Message
   Passer contract](../bridging/withdrawals.mdx#the-l2tol1messagepasser-contract) storage. Instead of making an MPT proof for a
