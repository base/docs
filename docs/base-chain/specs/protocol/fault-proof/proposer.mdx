...
[g-mpt]: ../../reference/glossary#merkle-patricia-trie
[header-withdrawals-root]: ../../upgrades/isthmus/exec-engine#l2tol1messagepasser-storage-root-in-header
...
queries the [rollup node](../consensus/index) for the latest output root derived from the latest
[finalized](../consensus/index#finalization-guarantees) L1 block. It then takes the output root and
...
1. The `withdrawal_storage_root` (`bytes32`) elevates the Merkle-Patricia-Trie ([MPT][g-mpt]) root of the [Message
   Passer contract](../bridging/withdrawals#the-l2tol1messagepasser-contract) storage. Instead of making an MPT proof for a
