...
[dispute game interface](stage-one/dispute-game-interface.mdx), in the L1 history up and till the specified `l1_head`.
...
This process is also known as the [L2 derivation process](../consensus/derivation.mdx),
and matches the processing in the [rollup node](../consensus/index.mdx) and
[execution-engine](../execution/index.mdx).
...
2. Compute the output-root, as the
   [proposer should compute it](proposer.mdx#l2-output-commitment-construction).
...
[computed output root](proposer.mdx#l2-output-commitment-construction).
...
> Also see [fault dispute-game specs](stage-one/fault-dispute-game.mdx) for fault dispute game system specifications,
> And [dispute-game-interface specs](stage-one/dispute-game-interface.mdx)
