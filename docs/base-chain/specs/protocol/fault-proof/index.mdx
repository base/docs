...
[dispute game interface](stage-one/dispute-game-interface), in the L1 history up and till the specified `l1_head`.
...
This process is also known as the [L2 derivation process](../consensus/derivation),
and matches the processing in the [rollup node](../consensus/index) and
[execution-engine](../execution/index).
...
2. Compute the output-root, as the
   [proposer should compute it](proposer#l2-output-commitment-construction).
...
[computed output root](proposer#l2-output-commitment-construction).
...
> Also see [fault dispute-game specs](stage-one/fault-dispute-game) for fault dispute game system specifications,
> And [dispute-game-interface specs](stage-one/dispute-game-interface)
