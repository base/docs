...
[g-output-root]: ../../../reference/glossary#l2-output-root

## Overview

The Fault Dispute Game (FDG) is a specific type of [dispute game](dispute-game-interface) that verifies the
...
determines if the game is [finalized](anchor-state-registry#finalized-game) and
["proper"](anchor-state-registry#proper-game) for purposes of [Bond
Distribution](bond-incentives#game-finalization). See [Anchor State Registry](anchor-state-registry) for more
details.

### Respected Game Type

A Fault Dispute Game must record whether its game type is respected at the time of its creation. See
[Respected Game Type](anchor-state-registry#respected-game-type) for more details.
...
[specification for Bonds](bond-incentives). Readers should understand basic game mechanics before
...
Finalization](bond-incentives#game-finalization).
