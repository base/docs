...
description: "Specification of the Fault Dispute Game (FDG), a bisection-based protocol for verifying the validity of L2 output roots via interactive dispute resolution."
---

[g-output-root]: ../../../reference/glossary.mdx#l2-output-root

## Overview

The Fault Dispute Game (FDG) is a specific type of [dispute game](dispute-game-interface.mdx) that verifies the
...
determines if the game is [finalized](anchor-state-registry.mdx#finalized-game) and
["proper"](anchor-state-registry.mdx#proper-game) for purposes of [Bond
Distribution](bond-incentives.mdx#game-finalization). See [Anchor State Registry](anchor-state-registry.mdx) for more
details.

### Respected Game Type

A Fault Dispute Game must record whether its game type is respected at the time of its creation. See
[Respected Game Type](anchor-state-registry.mdx#respected-game-type) for more details.
...
[specification for Bonds](bond-incentives.mdx). Readers should understand basic game mechanics before
...
Finalization](bond-incentives.mdx#game-finalization).
