...
- The game is a [Registered Game](anchor-state-registry#registered-game)
- The game is not a [Retired Game](anchor-state-registry#retired-game)
- The game has a game type that matches the current
  [Respected Game Type](anchor-state-registry#respected-game-type)
...
in progress (games that are not [Resolved Games](anchor-state-registry#resolved-game)).
...
- Withdrawal was proven against a game with a [Valid Claim](anchor-state-registry#valid-claim)
...
this commitment can be found [on this page](../proposer#l2-output-commitment-construction).
...
[blacklisting](anchor-state-registry#blacklisted-game) or by
[retirement](anchor-state-registry#retired-game) BEFORE they are considered to have
[Valid Claims](anchor-state-registry#valid-claim).

Proper Games that resolve in favor the Defender will be considered to have Valid Claims after the
[Dispute Game Finality Delay](anchor-state-registry#dispute-game-finality-delay-airgap) has
...
- [iASR-001](anchor-state-registry#iasr-001-games-are-represented-as-proper-games-accurately)
- [iASR-002](anchor-state-registry#iasr-002-all-valid-claims-are-truly-valid-claims)
...
[Dispute Game Finality Delay](anchor-state-registry#dispute-game-finality-delay-airgap) as per
...
[Respected Game Type](anchor-state-registry#respected-game-type) as per a call to
...
Returns the value of the current
[Retirement Timestamp](anchor-state-registry#retirement-timestamp) as per a call to
...
- MUST revert if the withdrawal is being proven against a game that is not a
  [Proper Game](anchor-state-registry#proper-game).
- MUST revert if the withdrawal is being proven against a game that is not a
  [Respected Game](anchor-state-registry#respected-game).
...
- MUST revert if the withdrawal being finalized was proven against a game that does not have a
  [Valid Claim](anchor-state-registry#valid-claim).
