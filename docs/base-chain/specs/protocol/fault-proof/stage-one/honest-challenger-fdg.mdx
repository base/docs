...
description: "Specification of the honest challenger agent in the Fault Dispute Game, defining correct behavior for supporting valid claims and disputing invalid ones."
---

## Overview

The honest challenger is an agent interacting in the [Fault Dispute Game](fault-dispute-game.mdx)
...
as well as a trace provider (ex: [Cannon](../cannon-fault-proof-vm.mdx)).
The trace provider must be configured with the [ABSOLUTE_PRESTATE](fault-dispute-game.mdx#execution-trace)
...
[state witness Hash](fault-dispute-game.mdx#claims) that agrees with the honest challenger's state witness hash for the
root claim.
...
index, then we disagree with the claim's stance by move to [defend](fault-dispute-game.mdx#defend).
Otherwise, the claim is [attacked](fault-dispute-game.mdx#attack).
...
[attack step](fault-dispute-game.mdx#step-types) when in response to such claims with
...
When the [chess clock](fault-dispute-game.mdx#game-clock) of a
[subgame root](fault-dispute-game.mdx#resolution) has run out, the subgame can be resolved.
