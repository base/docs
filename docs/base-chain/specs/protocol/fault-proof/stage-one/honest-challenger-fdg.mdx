...
The honest challenger is an agent interacting in the [Fault Dispute Game](fault-dispute-game)
...
as well as a trace provider (ex: [Cannon](../cannon-fault-proof-vm)).
The trace provider must be configured with the [ABSOLUTE_PRESTATE](fault-dispute-game#execution-trace)
...
[state witness Hash](fault-dispute-game#claims) that agrees with the honest challenger's state witness hash for the
root claim.
...
index, then we disagree with the claim's stance by move to [defend](fault-dispute-game#defend).
Otherwise, the claim is [attacked](fault-dispute-game#attack).
...
[attack step](fault-dispute-game#step-types) when in response to such claims with
...
When the [chess clock](fault-dispute-game#game-clock) of a
[subgame root](fault-dispute-game#resolution) has run out, the subgame can be resolved.
