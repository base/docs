...
Bonds is an add-on to the core [Fault Dispute Game](fault-dispute-game). The core game mechanics are
...
`isGameFinalized()`](anchor-state-registry#isgamefinalized) to return `true` before they can claim their bonds. This
...
3. Determine the bond distribution mode based on whether the [AnchorStateRegistry's
   `isGameProper()`](anchor-state-registry#isgameproper) returns `true`.
