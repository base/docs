...
The [rollup node](index.mdx) has an optional peer-to-peer (P2P) network service to improve the latency between
...
- V4 topic
  ...
  - `payload`: A SSZ-encoded `ExecutionPayload`, always the remaining bytes.
    - _Note_ - the `ExecutionPayload` is modified for the first time in Isthmus. See
      ["Update to `ExecutionPayload`"](../../upgrades/isthmus/exec-engine.mdx#update-to-executionpayload) in the Isthmus spec.
...
- The subsequent forkchoice-update ensures this block is recognized as "unsafe"
  (see [fork choice updated](derivation.mdx#engine-api-usage))
