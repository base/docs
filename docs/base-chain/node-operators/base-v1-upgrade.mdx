...
- **Client Version Mismatch:** If `web3_clientVersion` does not show `base/v0.7.0`, ensure you have pulled the latest `base/node` images and restarted with `docker compose down && docker compose up -d`.
- **P2P Connectivity:** Azul requires specific bootnodes. If your node isn't peering, verify that `BASE_NODE_P2P_BOOTNODES` is correctly mapped from your old `OP_NODE_P2P_BOOTNODES`.
- **Execution Layer Auth:** Ensure your `jwt.txt` is accessible by the new `base-reth-node` container. The path in `BASE_NODE_L2_ENGINE_AUTH` must be valid within the container environment.
## FAQ
