...
- **Issue**: Node has low peer count.  
    ...
    - **Action**: If behind NAT, configure the `--nat=extip:<your-ip>` flag via `ADDITIONAL_ARGS` in the `.env` file (see [Advanced Configuration](/base-chain/quickstart/run-base-node#improving-peer-connectivity)).
