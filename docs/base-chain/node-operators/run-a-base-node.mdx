...
If you're looking to harden your app and avoid rate-limiting for your users, please consider using an endpoint from one of our [partners](/base-chain/network-information/rpc-providers).
</Warning>
...
You'll need your own L1 RPC URL. This can be one that you run yourself, or via a third-party provider, such as our [partners](/base-chain/network-information/rpc-providers).
...
|---------|-------------------|----------------------------|----------------------------|----------------|----------|
| Base Mainnet | Azul activates May 28, 2026 18:00 UTC | `base-reth-node` v0.8.0+ after activation | `base-consensus` v0.8.0+ after activation | `BASE_NODE_*` | `optimism_syncStatus` on `BASE_NODE_RPC_PORT` |
