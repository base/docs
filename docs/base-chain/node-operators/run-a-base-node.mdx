...
<Note>
The [base](https://github.com/base/base) binary listens to the Flashblocks WebSocket stream and caches preconfirmation data. When Flashblocks-aware RPC methods are called, it returns data from this cache. For the full message schema and payload structure, see [Infrastructure Stream](/base-chain/flashblocks/api-reference#infrastructure-stream).
</Note>
...
Your Flashblocks-aware node supports all standard Ethereum JSON-RPC methods plus Flashblocks-specific methods and WebSocket subscriptions. See the [Flashblocks API Reference](/base-chain/flashblocks/api-reference) for the full list, including code examples and parameter details.
