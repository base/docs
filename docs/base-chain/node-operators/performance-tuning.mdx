...
4. Sufficient storage capacity calculated as:
  ...
  The table below shows an example calculation based on approximate current values for a Reth Archive node. **Check the linked sources for up-to-date figures before provisioning storage.**

  | Variable | Approximate value | Source |
  | :--- | :--- | :--- |
  | Chain size | ~9.8 TB | [Base Stats](https://base.org/stats) |
  | Snapshot size | ~1.8 TB | [Base Chain Data](https://basechaindata.vercel.app) |
  | Raw total | 2 × 9.8 + 1.8 = **21.4 TB** | — |
  | With 20% buffer | 21.4 × 1.2 = **~25.7 TB** | — |

  <Note>
  Chain size grows continuously. Add extra headroom (30–50%) when provisioning long-lived nodes to avoid resizing under load.
  </Note>
