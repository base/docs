...
Smart accounts on Ethereum today work around the protocol rather than inside it, and you pay for that in infrastructure.
...
Signature validation is pluggable. Authenticator contracts implement a single `authenticate` entrypoint, and the reference set covers secp256k1 (standard Ethereum keys), P-256, and WebAuthn.
