...
Endpoint limits do not change Base protocol capacity, but they can become the practical bottleneck for apps, wallets, indexers, and monitoring systems. For production use, choose an RPC provider from the [Base Services Hub](/get-started/base-services-hub) or [run a Base node](/base-chain/node-operators/run-a-base-node).
