...
This guide creates an Asset token, mints its initial supply, and verifies the balance onchain. To accept the token as payment in an app, continue with [Accept a B20 payment](/build-on-base/accept-b20-payment).
...
* [Accept a B20 payment in an app](/build-on-base/accept-b20-payment): wire this token into a checkout flow that tags each payment with an order ID and reconciles it from onchain events.
