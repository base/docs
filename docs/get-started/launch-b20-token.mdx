...
This guide creates an Asset token, mints its initial supply, and verifies the balance onchain. To accept the token as payment in an app, continue with [Accept B20 payments](/build-on-base/agentic-payments/accept-a-payment).
...
* [Accept B20 payments in an app](/build-on-base/agentic-payments/accept-a-payment): wire this token into a checkout flow that tags each payment with an order ID and reconciles it from onchain events.
