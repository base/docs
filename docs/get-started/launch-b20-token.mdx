...
A standard ERC-20 leaves that logic for you to build, audit, and maintain. With B20, you call the singleton [**B20 Factory**](/base-chain/specs/upgrades/beryl/b20/specification/reference/interfaces/IB20Factory) to create a token, fully configured, in a single transaction.
...
* Your token currently allows every transfer, by everyone, to anyone. That is a choice, not an accident — [configure policies in code](/base-chain/specs/upgrades/beryl/b20/specification/implementation/policy-configuration-in-code) to change it.
* Learn the full model in the [B20 token standard](/base-chain/specs/upgrades/beryl/b20/specification/overview).