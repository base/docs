...
<Steps>
  ...
  <Step title="Configure Wagmi for Base">
    ...
    <Info>
    The public `sepolia.base.org` endpoint is rate-limited and suitable for development only. Before going to production, replace it with a dedicated RPC URL from a [node provider](/base-chain/node-operators/node-providers) to avoid request failures under real traffic.
    </Info>
...
- **Go to mainnet** — add `base` to your `chains` array and transports in `config/wagmi.ts`, redeploy your contract to Base mainnet, and update `COUNTER_ADDRESS`. Switch the transport URL to a [node provider](/base-chain/node-operators/node-providers) — the public `mainnet.base.org` endpoint is rate-limited and not suitable for production traffic.
