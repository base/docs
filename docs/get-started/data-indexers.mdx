...
[Covalent](https://www.covalenthq.com/?utm_source=base&utm_medium=partner-docs) is a hosted blockchain data solution providing access to historical and current onchain data for [100+ supported blockchains](https://goldrush.dev/docs/chains), including [Base](https://goldrush.dev/docs/chains/base).
...
[The Indexing Company](https://www.indexing.co/) provides indexing as a service, capable of indexing any chain (EVM and non-EVM) with an RPC endpoint and integrating offchain data within the same infrastructure.
...
It currently serves historical onchain data, including event logs, transaction receipts, traces, and per-transaction state diffs.
...
It offers reusable **indexing primitives** (such as fault-tolerant RPC ingestors, custom processors and aggregations, re-org aware database integrations) to make it easy to receive, transform, store and access your onchain data.
