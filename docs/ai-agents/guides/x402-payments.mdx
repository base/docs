...
Base MCP can pay for x402-enabled HTTPS API requests from your Base Account. Your assistant sets a maximum USDC payment, Base MCP discovers the endpoint's x402 payment requirements, and you approve the payment before the request is completed.
...
<Steps>
  ...
  <Step title="You approve in Base Account">
    If payment is required, Base MCP returns an approval link and `requestId`. Open the link to review and approve the payment.
  </Step>
