...
<Tabs>
  <Tab title="Per-request (curl)">
    ...
    If the endpoint returns `402`, sign the EIP-712 payment payload from the response body and include it as a `PAYMENT-SIGNATURE` header on retry. Each call costs $0.001 USDC.
  </Tab>
