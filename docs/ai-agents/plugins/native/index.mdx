...
<Note>
Plugins that use `web_request` only reach protocols whose hostnames are on the Base MCP allowlist. Native plugin hostnames are covered for the hosted MCP. To call a protocol that isn't allowlisted, see [Build a custom plugin](/ai-agents/plugins/custom-plugins).
</Note>
...
<CardGroup cols={1}>
  <Card title="Build a custom plugin" icon="code" href="/ai-agents/plugins/custom-plugins">
    Write a markdown spec for a protocol with an HTTP tx-builder, sibling MCP server, or other Base MCP-compatible flow.
  </Card>
</CardGroup>
