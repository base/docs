...
<Note>
Base MCP's `web_request` tool can make GET and POST requests only to whitelisted partner APIs. Native plugin hosts are allowlisted for the hosted MCP. Custom plugin hosts usually are not, so custom plugins should expose GET endpoints to remain usable in Claude and ChatGPT consumer apps.
</Note>
...
| Sibling MCP server | Protocol already runs an MCP server with `prepare_*` tools | [Morpho](https://github.com/base/skills/blob/main/skills/base-mcp/plugins/morpho.md) |
...
<CardGroup cols={2}>
  ...
  <Card title="Native plugins" icon="puzzle-piece" href="/ai-agents/plugins/native">
    Reference implementations for the ordered-batch, sibling-MCP, and multi-endpoint patterns.
  </Card>
</CardGroup>
