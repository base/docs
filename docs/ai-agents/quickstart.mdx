    ...
    <Tabs>
      <Tab title="Claude">
        ...
        **Option 1: Paste this prompt into a new conversation**
        ...
        <Prompt description="Onboard via the SKILL.md">
        Use Base MCP. Fetch `https://base-a060aa97-youssef-update-agents.mintlify.app/ai-agents/skills/SKILL.md` using the `web_request` tool from Base MCP (not your built-in browser/fetch tool) and follow its onboarding exactly. Any subsequent `references/…` or `plugins/…` files referenced by `SKILL.md` should also be fetched via `web_request` from the same `https://base-a060aa97-youssef-update-agents.mintlify.app/ai-agents/skills/` path.
        </Prompt>
