...
2. **Wallet integration** — users must be able to connect and sign messages. See [Authenticate users](https://docs.cdp.coinbase.com/coinbase-wallet/guides/authenticate-users) or the [web React quickstart](https://docs.cdp.coinbase.com/coinbase-wallet/quickstart/web-react)
3. **Backend server** — to securely call the Base Verify API and keep your secret key private. For a similar frontend-to-backend signing pattern, see [Sign and verify typed data](https://docs.cdp.coinbase.com/coinbase-wallet/guides/sign-and-verify-typed-data)
