...
<ParamField body="permission" type="SpendPermission" required>
  The spend permission to revoke. This should be a SpendPermission object returned from [`requestSpendPermission`](/sdks/base-account/reference/spend-permission-utilities/requestSpendPermission) or fetched via [`fetchPermissions`](/sdks/base-account/reference/spend-permission-utilities/fetchPermissions).
...
<RequestExample>
```typescript User-initiated revoke lines wrap expandable
