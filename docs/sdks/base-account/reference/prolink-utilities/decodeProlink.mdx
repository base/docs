...
<ParamField body="payload" type="string" required>
  Base64url-encoded prolink payload to decode. This is the string returned by [`encodeProlink`](/sdks/base-account/reference/prolink-utilities/encodeProlink).
</ParamField>
...
```typescript Decode wallet_sendCalls lines wrap expandable
import { decodeProlink } from '@base-org/account';
...
```typescript Validate Before Execution lines wrap expandable
...
```typescript Inspect Spend Permission Request lines wrap expandable
...
```typescript Extract Capabilities lines wrap expandable
...
```typescript Display Transaction Details lines wrap expandable
...
<CardGroup cols={2}>
  <Card title="encodeProlink" icon="compress" href="/sdks/base-account/reference/prolink-utilities/encodeProlink">
