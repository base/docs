...
<RequestExample>
```typescript EOA Owner Wallet lines wrap expandable
...
```typescript Smart Owner Wallet lines wrap expandable
import { base } from '@base-org/account';
...
<ResponseExample>
```typescript Two Calls (Approval + Spend) lines wrap expandable
[
  {
...
```typescript Using prepareCharge lines wrap expandable
...
```typescript Using charge() lines wrap expandable
...
```typescript Before (prepareCharge) lines wrap expandable
...
```typescript After (charge) lines wrap expandable
