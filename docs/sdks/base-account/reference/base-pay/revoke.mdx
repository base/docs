...
<RequestExample>
```typescript Basic Revoke with Environment Variables lines wrap expandable
...
```typescript Revoke with Explicit Credentials lines wrap expandable
import { base } from '@base-org/account/node';
...
```typescript Revoke with Paymaster (Gasless) lines wrap expandable
import { base } from '@base-org/account/node';
...
```typescript User Cancellation Request lines wrap expandable
...
```typescript Policy Violation lines wrap expandable
...
```typescript Administrative Action lines wrap expandable
