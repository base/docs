...
<RequestExample>
```typescript Recommended: Basic Usage with Environment Variables lines wrap expandable
...
```typescript With Explicit Credentials lines wrap expandable
import { base } from '@base-org/account/node';
...
```typescript Complete Setup Flow lines wrap expandable
import { base } from '@base-org/account/node';
...
```typescript Advanced: Custom Wallet Names (Optional) lines wrap expandable
import { base } from '@base-org/account/node';
...
```typescript Idempotent Calls lines wrap expandable
...
```typescript Sponsor Gas for Charges
...
```typescript server.ts lines wrap expandable
...
```typescript Multiple Wallets lines wrap expandable
