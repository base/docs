...
<RequestExample>
```typescript Basic Status Check lines wrap expandable
...
```typescript Check Before Charging lines wrap expandable
import { base } from '@base-org/account';
...
<ResponseExample>
```typescript Active Subscription lines wrap expandable
{
  isSubscribed: true,
...
```typescript Partially Charged Subscription lines wrap expandable
{
  isSubscribed: true,
...
```typescript Cancelled Subscription lines wrap expandable
{
  isSubscribed: false,
  recurringCharge: "9.99",
...
```typescript Fully Charged Period lines wrap expandable
{
  isSubscribed: true,
...
```typescript Check Before Charge lines wrap expandable
...
```typescript Schedule Next Charge
const status = await base.subscription.getStatus({ id, testnet });
...
```typescript Display to User lines wrap expandable
const status = await base.subscription.getStatus({ id, testnet });
