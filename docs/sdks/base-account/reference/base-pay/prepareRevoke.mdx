...
<Note>
**Most developers should use [`revoke()`](/sdks/base-account/reference/base-pay/revoke) instead**, which handles execution automatically using CDP wallets. This function is for advanced use cases requiring custom transaction handling.
</Note>
...
For standard backend subscription management, use [`revoke()`](/sdks/base-account/reference/base-pay/revoke) instead.
...
<RequestExample>
```typescript EOA Wallet lines wrap expandable
...
```typescript Smart Wallet lines wrap expandable
import { base } from '@base-org/account';
...
```typescript With wallet_sendCalls lines wrap expandable
import { base } from '@base-org/account';
...
```typescript Using prepareRevoke lines wrap expandable
...
```typescript Using revoke() lines wrap expandable
...
```typescript Before (prepareRevoke) lines wrap expandable
...
<CardGroup cols={3}>
<Card title="Revoke" icon="bolt" href="/sdks/base-account/reference/base-pay/revoke">
