...
<RequestExample>
```typescript Basic Subscription lines wrap expandable
...
```typescript Weekly Subscription on Testnet lines wrap expandable
try {
  const subscription = await base.subscription.subscribe({
...
```typescript Annual Subscription lines wrap expandable
try {
  const subscription = await base.subscription.subscribe({
