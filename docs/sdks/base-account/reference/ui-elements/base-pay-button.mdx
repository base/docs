...
```tsx PaymentForm.tsx lines wrap expandable
...
```tsx Basic Payment lines wrap expandable
...
```tsx Payment with User Info Collection lines wrap expandable
const paymentOptions = {
  amount: '49.99',
  to: 'store.eth',
  payerInfo: {
...
```tsx Sizes and Variants lines wrap expandable
{/* Different sizes */}
...
```tsx Payment Result Handling lines wrap expandable
const handlePaymentResult = (result) => {
  if (result.success) {
    // Payment successful
...
```tsx Custom Click Handler lines wrap expandable
const handleClick = () => {
...
```tsx CheckoutPage.tsx lines wrap expandable
