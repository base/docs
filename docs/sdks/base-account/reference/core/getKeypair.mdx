...
<RequestExample>
```typescript Basic Usage lines wrap expandable
...
```typescript Get or Create Pattern lines wrap expandable
import { getKeypair, generateKeyPair } from '@base-org/account';
...
```typescript Key Lifecycle Management lines wrap expandable
class KeyManager {
