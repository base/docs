...
```typescript SDK Usage lines wrap expandable
...
<ResponseExample>
```json Full Capabilities Response lines wrap expandable
{
  "id": 1,
  "jsonrpc": "2.0",
  "result": {
...
```json Limited Capabilities Response lines wrap expandable
{
  "id": 1,
  "jsonrpc": "2.0",
  "result": {
...
```typescript Conditional Transaction Building lines wrap expandable
...
```typescript With wallet_connect lines wrap expandable
const capabilities = await provider.request({
  method: 'wallet_getCapabilities',
  params: [userAddress]
});
...
- [Capabilities Overview](/sdks/base-account/reference/core/capabilities/overview) - Complete guide to using capabilities
...
- [wallet_connect](/sdks/base-account/reference/core/provider-rpc-methods/wallet_connect) - Connect with authentication capabilities
