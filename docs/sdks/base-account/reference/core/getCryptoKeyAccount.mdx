...
```typescript Account Verification lines wrap expandable
const cryptoAccount = await getCryptoKeyAccount();
...
```typescript Account State Management lines wrap expandable
import { getCryptoKeyAccount } from '@base-org/account';
...
```typescript Integration with Provider lines wrap expandable
