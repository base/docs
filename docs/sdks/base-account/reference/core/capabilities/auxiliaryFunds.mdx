...
```typescript Balance Check with Auxiliary Funds lines wrap expandable
...
```typescript Wallet Capability Response lines wrap expandable
...
```typescript Without Auxiliary Funds lines wrap expandable
...
```typescript With Auxiliary Funds Support lines wrap expandable
async function checkCanExecuteTransaction(amount: bigint) {
  const capabilities = await provider.request({
    method: 'wallet_getCapabilities',
    params: [userAddress]
...
```typescript DeFi Applications lines wrap expandable
class DeFiManager {
...
```typescript E-Commerce Applications lines wrap expandable
...
```typescript Gaming Applications lines wrap expandable
...
- **[Paymaster Service](/sdks/base-account/reference/core/capabilities/paymasterService)**: For sponsored transactions
- **[Atomic](/sdks/base-account/reference/core/capabilities/atomic)**: For ensuring transaction success with auxiliary funds
- **[Flow Control](/sdks/base-account/reference/core/capabilities/flowControl)**: For handling auxiliary fund failures
