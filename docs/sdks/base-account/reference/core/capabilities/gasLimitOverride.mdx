...
```typescript Send calls with a gas limit override lines wrap expandable
...
```typescript Nondeterministic Gas Usage lines wrap expandable
...
```typescript Partial Gas Limit Specification lines wrap expandable
const result = await provider.request({
  method: "wallet_sendCalls",
  params: [{
    version: "1.0",
