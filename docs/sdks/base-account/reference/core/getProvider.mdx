...
```tsx Usage lines wrap expandable
...
For a full list of supported methods, see the [Provider Section](/sdks/base-account/reference/core/provider-rpc-methods/request-overview)

## Integration Examples

### With viem

```tsx With viem lines wrap expandable
...
```tsx With wagmi lines wrap expandable
...
```tsx Direct Provider Usage lines wrap expandable
// Request accounts
...
```tsx Event Handling lines wrap expandable
...
```tsx Error Handling lines wrap expandable
try {
  const accounts = await provider.request({ 
    method: 'eth_requestAccounts' 
  });
...
```tsx Provider Configuration lines wrap expandable
const sdk = createBaseAccountSDK({
  appName: 'My App Name',
  appLogoUrl: 'https://example.com/logo.png',
  appChainIds: [base.constants.CHAIN_IDS.base],
