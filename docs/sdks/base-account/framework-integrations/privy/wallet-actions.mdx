...
<CodeGroup>
```tsx Wallet Actions Component (components/sections/wallet-actions.tsx) lines wrap expandable
"use client";
...
<CodeGroup>
```tsx Sign Message lines wrap expandable
const handleSignMessage = async () => {
  if (!selectedWallet) return;
...
<CodeGroup>
```tsx Sign Typed Data lines wrap expandable
const handleSignTypedData = async () => {
  if (!selectedWallet) return;
...
<CodeGroup>
```tsx Send Transaction lines wrap expandable
const handleSendTransaction = async () => {
  if (!selectedWallet) return;
...
- [Batch Transactions](/sdks/base-account/improve-ux/batch-transactions)
- [Sponsor Gas](/sdks/base-account/improve-ux/sponsor-gas/paymasters)