...
<CodeGroup>
```tsx Spend Permissions Component (components/sections/spend-permissions.tsx) lines wrap expandable
"use client";
...
```tsx Create a Spend Permission lines wrap expandable
const permission = await requestSpendPermission({
  account,
  spender: spenderAddress,
  token: tokenAddress,
...
```tsx Allowance and Period Options
{
