...
<CodeGroup>
```tsx Sub Accounts Component (components/sections/sub-accounts.tsx) lines wrap expandable
"use client";
...
```tsx Create Sub Account (components/sections/sub-accounts.tsx) lines wrap expandable
await provider.request({
  method: 'wallet_addSubAccount',
  params: [{
    version: '1',
...
- [Sub Accounts Guide](/sdks/base-account/improve-ux/sub-accounts)
