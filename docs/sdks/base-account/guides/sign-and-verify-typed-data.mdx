...
```mermaid Signing Flow lines wrap expandable
sequenceDiagram
    participant User
    participant Browser
...
to spend user's funds (see [Spend Permissions](/sdks/base-account/improve-ux/spend-permissions))
...
```ts title="server/typed-data.ts" lines wrap expandable
