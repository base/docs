...
4. **Verify RPC Endpoints**: Use `curl` to check if the L2 client’s RPC endpoint is responding (see [Running a Node](/specifications/node-operators/run-a-node#running-a-node)). Also, verify your L1 endpoints are correct and accessible from the node server.
...
- **Issue**: Syncing is extremely slow.  
    - **Check**: Hardware specifications. Are you meeting the recommended specs (especially RAM and **NVMe SSD**) outlined in the [Node Performance](/specifications/node-operators/performance-tuning) guide? Disk I/O is often the bottleneck.  
...
- **Issue**: High CPU, RAM, or Disk I/O usage.
    - **Check**: Hardware specifications against recommendations in the [Node Performance](/specifications/node-operators/performance-tuning). Upgrade if necessary. Local NVMe SSDs are critical.
    - **Check**: Review client logs for specific errors or bottlenecks.

### Snapshot Restoration Problems

Refer to the [Snapshots](/specifications/node-operators/snapshots) guide for the correct procedure.
...
- **Issue**: Node has low peer count or cannot connect to any peers.  
    ...
    - **Action**: If behind NAT, configure the `--nat=extip:<your-ip>` flag via `ADDITIONAL_ARGS` in the `.env` file (see [Networking](/specifications/node-operators/run-a-node#networking)).
