...
<Warning>
**base/node has moved to [base/base](https://github.com/base/base).** As of **v1.3.0**, the operator node is published from base/base and all new releases ship there.

**base/node will be archived on September 4, 2026** and become read-only (no new issues, PRs, or releases), remaining available only as a reference for pre-v1.3.0 releases. For any new setup or upgrade, use base/base.
</Warning>

The root `docker-compose.yml` pulls `ghcr.io/base/node` by default; pass `--build` to compile the current source tree instead.
