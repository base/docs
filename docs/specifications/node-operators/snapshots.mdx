...
6. **Verify**: Monitor the node logs (`docker compose logs -f <service_name>`) or use the [sync monitoring](/specifications/node-operators/run-a-node#syncing) command to ensure the node starts syncing from the snapshot's block height.
...
If you are running the [historical proofs ExEx](/specifications/node-operators/run-a-node#enable-historical-proofs-rpcs), snapshots of the proofs database are available to skip the 24-48 hour backfill.
