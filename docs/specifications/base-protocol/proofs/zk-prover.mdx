---
title: "ZK Prover"
description: "Specification of the ZK prover, an offchain service that uses SP1 programs to produce permissionless proofs for checkpoint proposals and disputes."
---
...
required by `initializeWithInitData()`. See [Contracts](/specifications/base-protocol/proofs/proof-contracts) for the verifier-side framing.
...
```text Aggregation Program Inputs
...
```text Aggregate Output Fields lines wrap expandable
proverAddress
l1Head
l2PreRoot
...
[Contracts](/specifications/base-protocol/proofs/proof-contracts) terminology, `imageHash` is `ZK_RANGE_HASH`, and the aggregation
