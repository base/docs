...
description: "Specification of the L2 execution engine, detailing EIP-1559 parameters, fee vaults, Engine API usage, and execution layer behavior."
---
...
Starting with Holocene, the EIP-1559 parameters become [dynamically configurable](/base-chain/specs/upgrades/holocene/exec-engine#dynamic-eip-1559-parameters).

Starting with Jovian, a [configurable minimum base fee](/base-chain/specs/upgrades/jovian/exec-engine#minimum-base-fee) is introduced.
...
With Holocene, the `extraData` field [encodes the EIP-1559 parameters](/base-chain/specs/upgrades/holocene/exec-engine#dynamic-eip-1559-parameters).

With Jovian, the `extraData` encoding is extended to [include `minBaseFee`](/base-chain/specs/upgrades/jovian/exec-engine#minimum-base-fee).
...
[deposit-spec]: /specifications/base-protocol/bridging/deposits
...
| ------------------- | ------------------------------------------------------ |
| Sequencer Fee Vault | [`SequencerFeeVault`](/specifications/base-protocol/execution/predeploys#sequencerfeevault) |
| Base Fee Vault      | [`BaseFeeVault`](/specifications/base-protocol/execution/predeploys#basefeevault)           |
| L1 Fee Vault        | [`L1FeeVault`](/specifications/base-protocol/execution/predeploys#l1feevault)               |
...
```js Extended PayloadAttributesV2 lines wrap expandable
PayloadAttributesV2: {
    timestamp: QUANTITY
...
[rollup-driver]: /specifications/base-protocol/consensus/specification
...
```js Extended PayloadAttributesV3 lines wrap expandable
PayloadAttributesV3: {
    timestamp: QUANTITY
    prevRandao: DATA (32 bytes)
    suggestedFeeRecipient: DATA (20 bytes)
...
See [Dynamic EIP-1559 Parameters](/base-chain/specs/upgrades/holocene/exec-engine#dynamic-eip-1559-parameters) for details.

Starting with Jovian, the `minBaseFee` field is added. It must be `null` before Jovian.
See [Jovian Minimum Base Fee](/base-chain/specs/upgrades/jovian/exec-engine#minimum-base-fee) for details.
...
```js Extended GetPayloadV3 Response
{
...
```javascript SuperchainSignal Type
SuperchainSignal: {
...
[rollup node spec]: /specifications/base-protocol/consensus/specification
