---
title: "P2P"
description: "Specification of the rollup node peer-to-peer network, covering node discovery, gossip protocol, and unsafe block propagation."
---

## Overview

The [rollup node](/specifications/base-protocol/consensus/specification) has an optional peer-to-peer (P2P) network service to improve the latency between
...
- V4 topic
  ...
  - `payload`: A SSZ-encoded `ExecutionPayload`, always the remaining bytes.
    - _Note_ - the `ExecutionPayload` is modified for the first time in Isthmus. See
      ["Update to `ExecutionPayload`"](/base-chain/specs/upgrades/isthmus/exec-engine#update-to-executionpayload) in the Isthmus spec.
...
- The subsequent forkchoice-update ensures this block is recognized as "unsafe"
  (see [fork choice updated](/specifications/base-protocol/consensus/derivation#engine-api-usage))
