...
Validity Transactions let you submit a signed EIP-1559 transaction with conditions on Base state. Use them for conditional swaps, withdrawals, and other state-dependent actions.
...
Submit a signed raw transaction and a non-empty `validity` array. The current API supports storage and block-number predicates.
...
Validity Transactions use normal EIP-1559 fee fields. Eligibility does not reserve block space or guarantee inclusion.

## Lifetime and Privacy

Use a `block_number` predicate to set a maximum block. Use the sender's nonce when replacing a pending transaction.
