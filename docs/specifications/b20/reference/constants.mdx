---
title: "B20 Constants"
description: "B20 precompile addresses, role identifiers, policy scopes, and validation bounds."
---

*Role identifiers, policy-type identifiers, precompile addresses, and other fixed constants. See [`B20Constants`](https://github.com/base/base-std/blob/main/src/lib/B20Constants.sol) and [`StdPrecompiles`](https://github.com/base/base-std/blob/main/src/StdPrecompiles.sol).*

## Precompile addresses

*Fixed addresses of Base's singleton precompiles. See [`StdPrecompiles`](https://github.com/base/base-std/blob/main/src/StdPrecompiles.sol).*

| Name | Value | Purpose |
|---|---|---|
| `B20_FACTORY_ADDRESS` | `0xB20f000000000000000000000000000000000000` | Deploys and looks up B-20 tokens; every asset and stablecoin instance is created through the [`IB20Factory`](/specifications/b20/reference/interfaces#ib20factory) at this address. |
| `POLICY_REGISTRY_ADDRESS` | `0x8453000000000000000000000000000000000002` | Stores allowlist/blocklist/composite policies and answers `isAuthorized` checks consulted by every policy scope (see [Policies](/specifications/b20/concepts/policies)). |
...
*Role identifiers checked via `hasRole`. See [`B20Constants`](https://github.com/base/base-std/blob/main/src/lib/B20Constants.sol) and [`IB20`](/specifications/b20/reference/interfaces#ib20). Hex values are `keccak256` of the role name, verified with `cast keccak "<NAME>"` and cross-checked in `chisel`.*
...
*Policy scopes consulted by the PolicyRegistry. See [`B20Constants`](https://github.com/base/base-std/blob/main/src/lib/B20Constants.sol) and [Policies](/specifications/b20/concepts/policies). Hex values are `keccak256` of the policy name, verified with `cast keccak "<NAME>"` and cross-checked in `chisel`.*
...
*Fixed-point constants used by the multiplier/rebasing surface. See [`IB20Asset`](/specifications/b20/reference/interfaces#ib20asset).*
