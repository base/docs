...
- `stageUpdateAdmin` — sets the pending admin.
- `finalizeUpdateAdmin` — promotes the pending admin to current admin; reverts `NoPendingAdmin` if nothing is staged, or `Unauthorized` if the caller is not the staged address.
- `policyAdmin` — returns the current (active) admin.
