...
If a composite policy references `policyId` as an inverted child, the inverted evaluation also reflects the update immediately. No second write is needed on the composite or the token.
...
```solidity Add accounts to an allowlist
...
```solidity Remove an account from an allowlist
