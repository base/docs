...
The `uint64` policy ID currently bound to the scope. `0` means `ALWAYS_ALLOW`. Bit 63 may be set, indicating an inverted policy.
...
An unset scope reads as `0` (`ALWAYS_ALLOW`). To bind a different policy, the token admin calls `updatePolicy(policyScope, newPolicyId)`. An inverted ID is valid when its base policy exists; `policyExists` strips bit 63, so the registry resolves it correctly.
...
// id with bit 63 set means an inverted policy is bound
```
