...
|---|---|---|
| `from` | `address` | Account whose balance is being seized. Must be non-zero. May equal `address(this)` so a balance already stuck at the token can be recovered. |
| `to` | `address` | Destination for the seized balance. Must be non-zero, not this token's own address, and distinct from `from`. |
...
| `InvalidReceiver(to)` | `to` is `address(0)`, this token's own address, or `from == to`. |
...
<Note>
Seizing **from** the token's own address is allowed — pass `address(token)` as `from` to recover a balance that became stuck there. Seizing **to** the token's own address is not allowed and reverts with `InvalidReceiver`.
</Note>
