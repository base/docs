...
- A non-zero destination that is not the holder and not a B20-prefix address (typically a treasury).
...
| `InvalidReceiver(to)`                                  | `to` is `address(0)`, any B20-prefix address, or `from == to`.                                                              | Use a distinct, non-zero safekeeping address that is not a B20 token.              |
...
A balance already sitting at a B20 token address can still be recovered. `seizeWithMemo(token, treasury, amount, memo)` is allowed; seizing *to* a B20 address is not.
