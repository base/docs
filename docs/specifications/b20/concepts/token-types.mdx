---
title: "Token Types"
description: "Compare B20 Asset and Stablecoin variants, their creation parameters, and their capabilities."
---

*What Asset and Stablecoin are, how `createB20` seals the type into the address, and what each type adds on top of `IB20`. Roles and policies are shared across types; see [Roles and Pause](/specifications/b20/concepts/roles-and-pause) and [Policies](/specifications/b20/concepts/policies).*
...
After creation, the node reads the variant byte in the address and runs that variant's logic. See [`isB20`](/specifications/b20/reference/interfaces/ib20-factory/is-b20) and [`isB20Initialized`](/specifications/b20/reference/interfaces/ib20-factory/is-b20-initialized) for how callers detect a live token.

## 4. Asset

Asset is the general-purpose variant. That includes real-world assets (RWAs). It is not an RWA-only type. The type-specific surface is [`IB20Asset`](/specifications/b20/reference/interfaces/ib20-asset), which extends `IB20` at the same address.
...
After return, address byte `[10]` is `0x01`. `decimals()` is `6`. `currency()` is `"USD"`. The type-specific surface is [`IB20Stablecoin`](/specifications/b20/reference/interfaces/ib20-stablecoin). Calling `announce` on that address does not run Asset logic.
...
```mermaid 2 Creating an Asset Diagram lines wrap expandable highlight={1}
sequenceDiagram
    participant Issuer
    participant Factory
...
After return, address byte `[10]` is `0x00`. `decimals()` is `18`. [`IB20Asset`](/specifications/b20/reference/interfaces/ib20-asset) `announce` and `updateUIMultiplier` are live. There is no `currency()`.

### 6.3 A later call

A call to the token address does not choose the type again. The node reads byte `[10]` and runs that variant's logic.

```mermaid 3 A later call Diagram lines wrap expandable highlight={1}
flowchart TD
