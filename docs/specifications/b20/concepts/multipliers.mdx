---
title: "UI Multipliers"
description: "How B20 Asset UI multipliers scale displayed balances without changing raw balances."
---

*Mental model for the B20 Asset UI multiplier ([ERC-8056](https://eips.ethereum.org/EIPS/eip-8056)). How-to, events, and errors live in [`updateUIMultiplier`](/specifications/b20/reference/interfaces/ib20-asset/update-ui-multiplier). Asset-only; Stablecoin has no multiplier. See [Token Types](/specifications/b20/concepts/token-types).*
...
Operators typically wrap the schedule in `announce` so the split is a disclosed corporate action. See [`announce`](/specifications/b20/reference/interfaces/ib20-asset/announce).
...
```mermaid Scheduling lines wrap expandable highlight={1}
sequenceDiagram
...
- [Token Types](/specifications/b20/concepts/token-types) — Asset vs Stablecoin; multiplier is Asset-only.
- [Roles and Pause](/specifications/b20/concepts/roles-and-pause) — `OPERATOR_ROLE`.
- [`updateUIMultiplier`](/specifications/b20/reference/interfaces/ib20-asset/update-ui-multiplier) — how to schedule, cancel, override; events and errors.
- [`announce`](/specifications/b20/reference/interfaces/ib20-asset/announce) — disclosure wrapper around the schedule.
