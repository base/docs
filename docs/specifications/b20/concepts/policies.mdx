---
title: "Policies"
description: "How B20 policies apply shared allowlists, blocklists, and composite checks to token operations."
---

*How B20 reuses shared allowlists and blocklists for compliance checks. Roles and pause are a separate authorization layer; see [Roles and Pause](/specifications/b20/concepts/roles-and-pause).*
...
```mermaid 1 The registry and the token Diagram
flowchart TD
...
```mermaid 2 Policy types Diagram lines wrap expandable highlight={1}
flowchart TD
...
```mermaid 3 Inverting a policy Diagram
flowchart TD
...
```mermaid 1 Creating a policy Diagram
sequenceDiagram
    participant Creator
...
```mermaid 2 Updating a policy Diagram lines wrap expandable highlight={1}
sequenceDiagram
    participant Admin
...
```mermaid 1 Updating a scope Diagram lines wrap expandable highlight={1}
sequenceDiagram
    participant Admin
    participant Registry as Policy Registry
...
Most scopes deny when `isAuthorized` is `false` and revert `PolicyForbids`. `SEIZE_EXEMPT_POLICY` denies when `isAuthorized` is `true` and reverts `AccountNotSeizable`: an authorized account is seize-exempt.
...
| `SEIZE_EXEMPT_POLICY`      | `seizeWithMemo`. Unset (`ALWAYS_ALLOW`) means every account is seize-exempt, so none is seizable. | `from`                              | `true`                        | `AccountNotSeizable` |
...
```mermaid 1 One allowlist Diagram lines wrap expandable highlight={1}
sequenceDiagram
    participant Admin
    participant Registry as Policy Registry
    participant Token as B20 token
...
| `AccountNotSeizable(account)`          | `seizeWithMemo` `from` is still authorized (seize-exempt) under `SEIZE_EXEMPT_POLICY`    |
