---
title: "Roles and Pause"
description: "How B20 roles authorize privileged operations and pause vectors control token features."
---

*How B20 gates privileged operations with roles, and how pause freezes one class of operations without stopping the rest of the token. Policy checks are a separate authorization layer; see [Policies](/specifications/b20/concepts/policies).*
...
`OPERATOR_ROLE` exists only on Asset. See [Token Types](/specifications/b20/concepts/token-types). `approve` is not role-gated. Holder `transfer` is not role-gated. A holder can always move their own balance, subject to pause and policy.
...
```mermaid 4 Delegating administration lines wrap expandable highlight={1}
flowchart TD
...
```mermaid 1 All admin Diagram lines wrap expandable highlight={1}
sequenceDiagram
    participant Admin
    participant Token as B20 token
...
```mermaid 3 Example Diagram lines wrap expandable highlight={1}
sequenceDiagram
    participant Pauser
    participant Token as B20 token
    participant Caller
