...
- [Role-based access control](/specifications/b20/reference/interfaces/ib20) with built-in roles, user-defined roles in the role graph, and `renounceLastAdmin` for permanent admin-less operation
- [PolicyRegistry](/specifications/b20/reference/interfaces/i-policy-registry) singleton precompile with `BLOCKLIST`, `ALLOWLIST`, `UNION`, and `INTERSECT` policy types, two-step admin transfer, and the `ALWAYS_ALLOW` and `ALWAYS_BLOCK` built-ins
- [Six policy scopes](/specifications/b20/reference/interfaces/ib20/policy-id#policy-scopes) on every token, covering transfer sender, receiver, and executor, mint receiver, and seize holder and receiver
...
- [Granular pausing](/specifications/b20/specification-overview#pause-vectors) by `PausableFeature`: `TRANSFER`, `MINT`, `BURN`, and `SEIZE`
...
- [B20Factory](/specifications/b20/reference/interfaces/ib20-factory) singleton precompile with deterministic, variant-encoding addresses and `initCalls` bootstrap semantics
- ActivationRegistry gating for state-changing PolicyRegistry calls
- The [`ASSET` variant](/specifications/b20/reference/interfaces/ib20-asset): `OPERATOR_ROLE`, WAD-precision UI multipliers with scheduled and instant updates, announcements, `batchMint`, and issuer-defined extra metadata
- The [`STABLECOIN` variant](/specifications/b20/reference/interfaces/ib20-stablecoin): fixed 6 decimals and a `currency()` code set once at creation
