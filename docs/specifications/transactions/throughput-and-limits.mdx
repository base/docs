...
Endpoint limits do not change Base protocol capacity, but they can become the practical bottleneck for apps, wallets, indexers, and monitoring systems. For production use, choose an RPC provider from the [Builder Stack](/get-started/builder-stack) or [run a Base node](/specifications/node-operators/run-a-node).
