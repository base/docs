...
<Note>
Base has a [minimum base fee](/specifications/transactions/network-fees#minimum-base-fee). Transactions with `maxFeePerGas` below this value will never be included, since the base fee cannot drop below the minimum.
</Note>
...
Base enforces a [per-transaction gas maximum](/specifications/transactions/throughput-and-limits#per-transaction-gas-maximum) of **16,777,216 gas (2^24)**. Transactions specifying a higher gas limit are rejected during block validation.
...
Base produces blocks every 2 seconds, but [Flashblocks](/specifications/transactions/transaction-ordering#flashblocks) provide preconfirmations every 200ms.
...
See [Transaction Finality](/specifications/transactions/transaction-finality) for more details.
