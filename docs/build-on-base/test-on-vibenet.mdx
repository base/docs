...
```javascript newHeads subscription lines wrap expandable
...
<Note>
This public WebSocket is Vibenet only. Base does not provide a free public WebSocket RPC endpoint for Base Mainnet or Base Sepolia; the public endpoints there are HTTP only. For WebSocket access on Sepolia or Mainnet, run your own node or use a WebSocket-capable provider from the [Builder Stack](/get-started/builder-stack).
</Note>
