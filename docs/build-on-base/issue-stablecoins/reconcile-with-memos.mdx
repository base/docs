...
<Warning>
`TRANSFER_EXECUTOR_POLICY` now applies to `transferWithMemo` and `transferFromWithMemo` (and all other transfer entrypoints), including when `msg.sender` is the token holder. If your token has a restrictive executor policy set, callers must be authorized as initiators to use memo transfers.
</Warning>
