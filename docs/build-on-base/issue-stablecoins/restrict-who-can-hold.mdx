...
<Warning>
`transfer`, `transferFrom`, and `mint` reject the token's own address as a recipient with `InvalidReceiver`. Do not pass `token.address` as the `to` argument — a balance credited there has no holder-side way to move it.
</Warning>
