...
```solidity Attach the blocklist to the seize scope
token.updatePolicy(token.SEIZE_EXEMPT_POLICY(), blocklistId);
```

Attach a blocklist, never an allowlist, to `SEIZE_EXEMPT_POLICY`: an empty allowlist authorizes nobody, which would make every holder seizable. See [Recover Funds](/build-on-base/issue-stablecoins/recover-funds) for the seize itself.

## See Also

<CardGroup cols={2}>
  <Card title="Recover Funds" icon="rotate-left" href="/build-on-base/issue-stablecoins/recover-funds">
    Move a blocked balance to safekeeping with seizeWithMemo.
  </Card>
  <Card title="Pause Activity" icon="circle-pause" href="/build-on-base/issue-stablecoins/pause-activity">
