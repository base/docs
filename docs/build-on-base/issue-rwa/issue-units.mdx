...
<Warning>
The entire batch reverts if lengths differ, a recipient fails policy, or total supply would exceed the cap. Passing `address(0)` or the token's own address as any recipient also reverts with `InvalidReceiver` — credits to the token contract itself are unrecoverable.
</Warning>
