...
<Note>
This example issues additional shares. It does not distribute a cash dividend. A reinvested dividend that only rescales displayed balances is a multiplier update, covered below and in [Apply a Multiplier](/build-on-base/issue-rwa/apply-a-multiplier).
</Note>
...
<Note>
`TRANSFER_EXECUTOR_POLICY` now applies to every transfer path — including `transfer`, `transferFrom`, and their memo variants — even when `msg.sender == from`. If an inner call triggers a transfer and you have a restrictive executor policy configured, ensure the announcing operator is authorized as an executor.
</Note>
