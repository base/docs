...
<Warning>
`burnBlocked` is deprecated. It destroys units directly from a holder denied under `TRANSFER_SENDER_POLICY`, needs `BURN_BLOCKED_ROLE`, and emits no `Seized` event, so there is no onchain record that the units were taken rather than redeemed. Prefer seize, then burn.
</Warning>
