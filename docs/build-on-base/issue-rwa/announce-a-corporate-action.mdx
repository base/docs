...
On success the asset emits `Announcement`, `Transfer(operator, address(0), amount)`, `Memo`, then `EndAnnouncement`. `totalSupply` decreases. To take units from a holder and then destroy them, [seize](/build-on-base/issue-rwa/seize-and-cancel-units) first, then announce the burn from the treasury.
