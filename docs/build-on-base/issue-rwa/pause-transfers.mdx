...
<Note>
`transfer`, `transferFrom`, and `mint` all revert with `InvalidReceiver` when `to` is `address(0)` **or** the token's own address. Sending to the token contract itself would lock the credited balance with no holder-side way to recover it.
</Note>
