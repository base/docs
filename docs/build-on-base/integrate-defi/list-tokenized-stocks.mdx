...
- **Onchain:** [`name`](/specifications/b20/reference/interfaces), [`symbol`](/specifications/b20/reference/interfaces), [`decimals`](/specifications/b20/reference/interfaces), and [`contractURI`](/specifications/b20/reference/interfaces) ([ERC-7572](https://eips.ethereum.org/EIPS/eip-7572) metadata).
...
- **Scheduled (ERC-8056):** set a future-dated change with [`updateUIMultiplier`](/specifications/b20/reference/interfaces), which gives advance onchain notice. Use this for routine corporate actions.
- **Instant (deprecated):** [`updateMultiplier`](/specifications/b20/reference/interfaces) applies immediately and clears any pending scheduled change. It is a retained emergency failsafe; prefer the scheduled path.
- **Cancellation:** clear a pending scheduled update with [`cancelUIMultiplierUpdate`](/specifications/b20/reference/interfaces).
...
|----------|-------------|
| [`scaledBalanceOf(account)`](/specifications/b20/reference/interfaces) | Raw balance × multiplier |
| [`toScaledBalance(raw)`](/specifications/b20/reference/interfaces) | Convert raw amount to scaled |
| [`toRawBalance(scaled)`](/specifications/b20/reference/interfaces) | Convert scaled amount to raw |

### Policies

To comply with any regulatory requirements that apply to a particular asset, [policies](/specifications/b20/reference/interfaces) that manage allowlists and blocklists may be implemented. Policies determine whether a transfer is allowed or rejected.

The B20 contract provides the [`isAuthorized(policyID, account)`](/specifications/b20/reference/interfaces) function, which you can use to determine whether a specific account is allowed to transfer funds.

<Note>
The standard [`approve()`](/specifications/b20/reference/interfaces) function is not policy gated. Checking whether a quantity of funds is approved to be transferred does not guarantee that the funds aren't blocked by a policy.
</Note>

### Pauses

Onchain [pauses](/specifications/b20/reference/interfaces) are unlikely, but be aware that B20s allow specific functions within a contract to be paused. Monitor these to maintain an accurate understanding of whether funds can be transferred at a given point in time.

### Announcements

Sensitive operations are wrapped in onchain [announcement events](/specifications/b20/reference/interfaces): [`announce`](/specifications/b20/reference/interfaces) emits [`Announcement (id, description, uri)`](/specifications/b20/reference/interfaces), then [`EndAnnouncement`](/specifications/b20/reference/interfaces). Integrators index these to catch corporate actions as they execute.
...
**Admin actions:** admin operations and `updateMultiplier` ([`OPERATOR_ROLE`](/specifications/b20/reference/interfaces)) execute immediately when the role holder calls; the B20 standard has no built-in timelock. The `Announcement` and `EndAnnouncement` events are public notice, not an enforced delay. Any timelock or multisig is applied by the issuer at the governance layer.

### Extra Metadata

Issuers can store arbitrary key/value data onchain via [`extraMetadata(key)`](/specifications/b20/reference/interfaces) (for example, security identifiers such as ISIN and CUSIP).

### Name and Symbol

Name and symbol are updatable onchain ([`updateName`](/specifications/b20/reference/interfaces), [`updateSymbol`](/specifications/b20/reference/interfaces)), so the token can track offchain changes to the underlying without redeploying.

### Memos

[`transferWithMemo`](/specifications/b20/reference/interfaces) and [`transferFromWithMemo`](/specifications/b20/reference/interfaces) attach a `bytes32` reference to an individual transfer (emitted as a `Memo` event), for annotating transfers with offchain data for reconciliation and reporting.
...
The multiplier is sourced per token and is WAD-scaled (a fixed-point number with 18 decimals). To get the real factor, divide the onchain value by that scale; call [`WAD_PRECISION()`](/specifications/b20/reference/interfaces) to read the scale (it returns `1e18`).
...
Since prices are total-return (multiplier-adjusted), reconstruct the series consistently by applying the multiplier history ([`MultiplierUpdated`](/specifications/b20/reference/interfaces) events, emitted by both scheduled and instant multiplier changes) if starting from raw share prices.
...
- [B20 Standard](/specifications/b20/overview)
