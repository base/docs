...
<Tip>
The public [Tokenized Stocks API](/sdks/tokenized-stocks/overview) returns current listings, contract addresses, supplies, multipliers, and NAV prices in a single read-only request, with no API key required.
</Tip>
...
The [Tokenized Stocks API](/sdks/tokenized-stocks/api-reference/list-tokenized-stocks) also returns each token's latest Chainlink value as `nav_price`, along with `nav_price_updated_at` and the current `multiplier`. Apply the same staleness checks to `nav_price_updated_at` that you would to `updatedAt`.
