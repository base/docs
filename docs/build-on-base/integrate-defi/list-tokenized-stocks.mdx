...
- **API:** the [List tokenized stocks](/sdks/tokenized-stocks/overview#endpoints) endpoint returns each token's contract address, symbol, name, decimals, icon URL, and ISIN.
...
The [Tokenized Stocks API](/sdks/tokenized-stocks/overview#endpoints) also returns each token's latest Chainlink value as `nav_price`, along with `nav_price_updated_at` and the current `multiplier`. Apply the same staleness checks to `nav_price_updated_at` that you would to `updatedAt`.
...
To fetch these addresses programmatically, use the [List tokenized stocks](/sdks/tokenized-stocks/overview#endpoints) endpoint for token contracts and the [List protocol contract addresses](/sdks/tokenized-stocks/overview#endpoints) endpoint for protocol contracts.
