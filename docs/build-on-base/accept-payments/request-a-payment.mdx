...
export async function sendUsdc(merchant: Address) {
  ...
  <Tab title="B20 with Memo">
    ...
    <Warning>
    B20 rejects the token's own address as a transfer destination. Sending to `address(0)` or to the token contract itself reverts with `InvalidReceiver`. Always use a genuine recipient address (wallet or merchant contract).
    </Warning>
