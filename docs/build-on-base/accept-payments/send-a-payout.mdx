...
<Warning>
B20 tokens reject the token contract's own address as a recipient. A payout batch that includes a B20 token's own address in `recipients` will revert with `InvalidReceiver`. Validate recipient addresses before submission.
</Warning>
