...
<Warning>
B20 tokens enforce `TRANSFER_EXECUTOR_POLICY` on every transfer path — including `transferFrom` calls where `msg.sender == from`. If the token has a restrictive executor policy, your payout contract's address must be authorized as an executor, or all transfers will revert with `PolicyForbids(TRANSFER_EXECUTOR_POLICY, ...)`.
</Warning>
