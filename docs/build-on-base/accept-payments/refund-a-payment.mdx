...
<Warning>
B20 tokens with a restrictive `TRANSFER_EXECUTOR_POLICY` now apply that policy to every transfer call — including direct `transfer` and `transferWithMemo` — not only delegated `transferFrom` paths. If your merchant account is not authorized as an executor on the token, the refund transfer will revert. Confirm your account is on the token's executor allowlist before broadcasting a refund.
</Warning>
