...
<Note>
B20 tokens reject the token's own address as a transfer destination (`to == address(this)`). Ensure your merchant address is a normal account or contract, not the token contract itself — such transfers revert with `InvalidReceiver`.
</Note>
