...
description: "Specification for Cobalt's canonical 200ms blocks, including BaseTime, derivation, validation, and RPC behavior."
tag: "Vibenet"
---
...
Cobalt is targeting Base Sepolia and Base Mainnet in September 2026; exact activation times and required client versions remain undecided.
</Warning>

## Summary

Cobalt changes Base block production from one canonical block every two seconds to five complete canonical blocks per second. Each 200ms block has its own block number, hash, state root, receipts, forkchoice updates, and unsafe, safe, and finalized lifecycle.

Cobalt replaces [Flashblocks](/specifications/flashblocks), which publish incremental pending-state updates for a single block. As part of the Cobalt rollout, Base will stop producing Flashblocks and instead produce a canonical block every 200ms. Applications using Flashblocks must migrate to canonical block and RPC streams.

Cobalt keeps the Ethereum block header and its seconds-based `timestamp` unchanged. A BaseTime metadata deposit supplies the sub-second component. Together, these values identify a canonical block's full millisecond timestamp.

<Info>
If your application uses Flashblocks, see [Migrate From Flashblocks](/upgrades/cobalt/migrate-from-flashblocks) for the required API changes.
</Info>
...
| `sepolia` | Planning for September 2026 | `TBD` | `TBD` |
| `mainnet` | Planning for September 2026 | `TBD` | `TBD` |

Exact Base Sepolia and Base Mainnet activation timestamps have not been scheduled.

## Execution

### Full block timestamp

Cobalt leaves `block.header.timestamp` as Unix time in whole seconds. For an activated block `b`, its full timestamp is:
...
For payloads at or after Cobalt activation, `BasePayloadAttributes.timestampMillisPart` **MUST** be present and equal `0`, `200`, `400`, `600`, or `800`. Before activation, it **MUST NOT** be present.
...
After activation, the derivation pipeline computes each block's timestamp from the Cobalt activation schedule and absolute L2 block number. It does not read the millisecond part from batch data or derive it from the local wall clock. The sequence is:
...
<Note>
The RPC behavior below is planned for Cobalt and is not available on production endpoints.
</Note>

### Seconds compatibility

Cobalt keeps the existing `timestamp` JSON-RPC field in Unix seconds. Engine timestamps, transaction-validity timestamps, `eth_call` timestamps, and EVM `block.timestamp` also remain seconds-based. RPC responses do not expose the internal `timestampMillisPart` field.

After the Cobalt rollout, use canonical block responses and subscriptions such as `eth_subscribe("newHeads")` for sub-second updates. Flashblocks streams will stop.
...
As with block responses, pruned or unprovenanced transaction and log data will omit `blockTimestampMs`. The fields are optional so pre-Cobalt history and clients without the required body data remain representable.

### Tooling

Foundry's `AnyRpcBlock` and `OtherFields` paths can preserve an unknown block-level `timestampMs`, while EVM execution continues to use seconds. Plain Alloy `AnyRpcHeader` drops unknown fields; consumers that need Cobalt timestamps can use `WithOtherFields<AnyRpcHeader>` or a typed Base response. Locally mined Anvil blocks are not expected to produce BaseTime metadata in the initial rollout.
