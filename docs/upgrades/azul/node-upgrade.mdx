...
<Check>
Azul activated on mainnet on **May 28, 2026 18:00 UTC** (`1779991200`). See the [required software versions](/upgrades/azul/overview#required-software) and [full activation timestamps](/upgrades/azul/overview#activation-timestamps) on the Azul overview.
</Check>
...
5. Bootstrap from a [Reth snapshot](/specifications/node-operators/snapshots) to avoid a full sync.
...
- **What if I'm on `op-geth` or `nethermind`?** You need to switch to `base-reth-node`. Use a [Reth snapshot](/specifications/node-operators/snapshots) to bootstrap.
