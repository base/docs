...
[gas-market]: /specifications/base-protocol/bridging/deposits#default-values
...
```json FlashblocksMetadata Before Azul lines wrap expandable
{
...
```json FlashblocksMetadata After Azul
{
  "block_number": 43403718,
  "access_list": null
}
...
- `precompiles` reflects the active EVM precompile set for that fork. This includes the standard
  ...
  [precompiles specification](/specifications/base-protocol/execution/precompiles).
...
[predeploys specification](/specifications/base-protocol/execution/predeploys) are not serialized into
