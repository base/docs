...
description: "Derivation changes in the Fjord upgrade, updating protocol parameters for max sequencer drift, channel bank size, and adding brotli compression support."
---
...
The usage of `MAX_RLP_BYTES_PER_CHANNEL` is defined in [Channel Format](/specifications/base-protocol/consensus/derivation#channel-format).
The usage of `MAX_CHANNEL_BANK_SIZE` is defined in [Channel Bank Pruning](/specifications/base-protocol/consensus/derivation#pruning).
...
[legacy-channel-format]: /specifications/base-protocol/consensus/derivation#channel-format
...
```text Versioned Channel Encoding
