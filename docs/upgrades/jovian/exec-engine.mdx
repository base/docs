...
The minimum base fee is configured via `SystemConfig` (see [System Configuration](/specifications/base-protocol/consensus/derivation#system-configuration)) and enforced by the execution engine
...
```javascript Minimum Base Fee Clamp
...
The Engine API [`PayloadAttributesV3`](/specifications/base-protocol/execution/l2-execution-engine#extended-payloadattributesv3) is extended with a new
field `minBaseFee`. The existing `eip1559Params` remains 8 bytes (Holocene format).

```text Extended PayloadAttributesV3 lines wrap expandable
PayloadAttributesV3: {
    timestamp: QUANTITY
...
```python DA Footprint Calculation lines wrap expandable
