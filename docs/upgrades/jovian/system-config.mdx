...
```solidity SystemConfig.sol
function setMinBaseFee(uint64 minBaseFee) external onlyOwner;
```

### `ConfigUpdate`

When the configuration is updated, a [`ConfigUpdate`](/specifications/base-protocol/consensus/derivation#system-config-updates) event
