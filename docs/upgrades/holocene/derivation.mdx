...
that the _uncompressed_ size of any given channel is _clipped_ to `MAX_RLP_BYTES_PER_CHANNEL` [during decompression](/specifications/base-protocol/consensus/derivation#channel-format).
...
So the following changes are made to the [Bedrock Batch Queue](/specifications/base-protocol/consensus/derivation#batch-queue):
...
the engine to use the [canyon base fee parameter constants](/specifications/base-protocol/execution/l2-execution-engine#1559-parameters). This
...
[original sync start description](/specifications/base-protocol/consensus/derivation#finding-the-sync-starting-point) for details),
