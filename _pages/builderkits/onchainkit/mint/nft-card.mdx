...
Before using the `NFTCard` component, ensure you've completed the [Getting Started](/builderkits/onchainkit/getting-started) steps.

To use the `NFTCard` component, you'll need to provide an API Key in `OnchainKitProvider`. You can get one following our [Getting Started](/builderkits/onchainkit/getting-started#get-your-public-api-key) steps.
...
We recommend style customization by setting a custom [OnchainKit theme](/builderkits/onchainkit/guides/themes#custom-theme). You can also override individual component styles using `className`.
...
The default `NFTCard` implementation uses Coinbase Developer Platform to provide the data and requires an API key.  If you would like to use your own data, you can use the `useNFTData` prop to pass in a hook which fetches the NFT data from a source of your choice.  As long as this hook returns an object with the same shape as the [`NFTData`](/builderkits/onchainkit/mint/types#nftdata) type, the NFTCard will render.
...
You can use our NFT [`LifecycleStatus`](/builderkits/onchainkit/mint/types#lifecyclestatus) and the `onStatus` prop to listen to transaction states.
...
- [`LifecycleStatus`](/builderkits/onchainkit/mint/types#lifecyclestatus)
- [`NFTCardReact`](/builderkits/onchainkit/mint/types#nftcardreact)
- [`NFTData`](/builderkits/onchainkit/mint/types#nftdata)
